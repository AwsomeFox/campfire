...
describe("encounters (e2e)", () => {
  ...
  describe("encounter auto-add respects character lifecycle status (issue #115)", () => {
    ...
    it("only active characters are auto-added; dead/retired/draft are skipped", async () => {
      const server = ctx.app.getHttpServer();
      const res = await request(server)
        .post(`/api/v1/campaigns/${statusCampId}/encounters`)
        .set(dm)
        .send({ name: "New Fight", hidden: false });
      expect(res.status).toBe(201);
      const charIds = (
        res.body.combatants as Array<{ characterId: number | null }>
      ).map((c) => c.characterId);
    ...
    it("marking a PC active again re-includes it in the next encounter", async () => {
      ...
      const patch = await request(server)
        .patch(`/api/v1/characters/${deadId}`)
        .set(dm)
        .send({ status: "active" });
      ...
      const res = await request(server)
        .post(`/api/v1/campaigns/${statusCampId}/encounters`)
        .set(dm)
        .send({ name: "Second Fight", hidden: false });
      expect(res.status).toBe(201);
      const charIds = (
        res.body.combatants as Array<{ characterId: number | null }>
      ).map((c) => c.characterId);
  ...
  describe("full combat flow", () => {
    ...
    beforeAll(async () => {
      ...
      const res = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "The Big Fight", hidden: false });
    ...
    it("GET /campaigns/:id/encounters lists it, filterable by status", async () => {
      ...
      expect(
        listRes.body.some((e: { id: number }) => e.id === encounterId),
      ).toBe(true);
      ...
      expect(
        filteredRes.body.every(
          (e: { status: string }) => e.status === "preparing",
        ),
      ).toBe(true);
    });
    ...
    it("roll-initiative fills only null initiatives", async () => {
      ...
      for (const c of rollRes.body.combatants as Array<{
        id: number;
      ...
      const ogre = (
        rollRes.body.combatants as Array<{ id: number; initiative: number }>
      ).find((c) => c.id === monsterId);
    ...
    it("dm starts the encounter: running, round=1, turnIndex=0, sorted by initiative desc", async () => {
      ...
      const initiatives = (
        res.body.combatants as Array<{ initiative: number }>
      ).map((c) => c.initiative);
    ...
    it("next-turn advances turnIndex and wraps to round+1", async () => {
      ...
      let lastBody: { round: number; turnIndex: number } = {
        round: 1,
        turnIndex: 0,
      };
    ...
    it("owning player can adjust their own combatant hp", async () => {
      ...
      const res = await request(server)
        .patch(
          `/api/v1/encounters/${encounterId}/combatants/${ariaCombatantId}`,
        )
        .set(player)
        .send({ hpDelta: -5 });
      expect(res.status).toBe(200);
    ...
    it("applies direct 5e damage type, save-half, immunity, vulnerability, and crit dice math (issue #605)", async () => {
      ...
      const source = await db
        .select({ packId: ruleEntries.packId })
        .from(ruleEntries)
        .where(eq(ruleEntries.id, ruleEntryId))
        .get();
      ...
      const savedEvents = await request(server)
        .get(`/api/v1/encounters/${encounterId}/events`)
        .set(dm);
      const savedEvent = savedEvents.body.find((event: { detail: string }) =>
        /18 fire.*saved for half/.test(event.detail),
      );
      ...
      const events = await request(server)
        ...
        .set(dm);
      expect(
        events.body.some((event: { detail: string }) =>
          /took 0 damage.*immune/.test(event.detail),
        ),
      ).toBe(true);
      ...
      const normalizedEvents = await request(server)
        .get(`/api/v1/encounters/${encounterId}/events`)
        .set(dm);
      expect(
        normalizedEvents.body.some((event: { detail: string }) =>
          /4 fire.*resistant/.test(event.detail),
        ),
      ).toBe(true);
      ...
      const unknownType = await request(server)
        .patch(`/api/v1/encounters/${encounterId}/combatants/${targetId}`)
        .set(dm)
      ...
      expect(meaninglessDiceSubtotal.body.message).toContain(
        "requires a critical hit",
      );
      ...
      expect(conflictingHpSet.body.message).toContain(
        "cannot be combined with hpSet",
      );
      ...
      const fullSaveHit = await request(server)
        .patch(
          `/api/v1/encounters/${encounterId}/combatants/${fullSaveTarget.body.id}`,
        )
        .set(dm)
        .send({ hpDelta: -10, damageType: "fire" });
      const halfSaveHit = await request(server)
        .patch(
          `/api/v1/encounters/${encounterId}/combatants/${halfSaveTarget.body.id}`,
        )
        .set(dm)
      ...
      const qualifiedHit = await request(server)
        .patch(
          `/api/v1/encounters/${encounterId}/combatants/${qualifiedTarget.body.id}`,
        )
        .set(dm)
    ...
    it("owning player cannot inject an arbitrary free-text condition (issue #495)", async () => {
      ...
      const after = await request(server)
        ...
        .set(dm);
      const aria = (
        after.body.combatants as Array<{ id: number; conditions: string[] }>
      ).find((c) => c.id === ariaCombatantId)!;
    ...
    it("unknown key in combatant PATCH body (e.g. hpCurrent instead of hpSet/hpDelta) -> 400, not a silent no-op", async () => {
      ...
      const before = await request(server)
        ...
        .set(dm);
      const ariaBefore = (
        before.body.combatants as Array<{ id: number; hpCurrent: number }>
      ).find((c) => c.id === ariaCombatantId)!;
      ...
      const after = await request(server)
        ...
        .set(dm);
      const ariaAfter = (
        after.body.combatants as Array<{ id: number; hpCurrent: number }>
      ).find((c) => c.id === ariaCombatantId)!;
    ...
    it("dm removes a combatant", async () => {
      const server = ctx.app.getHttpServer();
      const res = await request(server)
        .delete(`/api/v1/encounters/${encounterId}/combatants/${ruleMonsterId}`)
        .set(dm);
      expect(res.status).toBe(200);
      ...
      const getRes = await request(server)
        ...
        .set(dm);
      expect(
        getRes.body.combatants.some(
          (c: { id: number }) => c.id === ruleMonsterId,
        ),
      ).toBe(false);
    });
    ...
    it("combatant routes 404 when encounterId doesn't own the combatant (cross-parent-id pin)", async () => {
      ...
      const otherEncRes = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Unrelated Fight", hidden: false });
      ...
      const wrongPatch = await request(server)
        .patch(
          `/api/v1/encounters/${otherEncounterId}/combatants/${ariaCombatantId}`,
        )
        .set(dm)
        .send({ hpDelta: -1 });
      expect(wrongPatch.status).toBe(404);

      const wrongDelete = await request(server)
        .delete(
          `/api/v1/encounters/${otherEncounterId}/combatants/${ariaCombatantId}`,
        )
        .set(dm);
    ...
    it("characterId belonging to a different campaign is rejected 404 (round-2 finding #5)", async () => {
      ...
      const otherCampRes = await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
    ...
    it("/next-turn on a non-running encounter is rejected 400 (state machine, verify existing guard)", async () => {
      ...
      const freshRes = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Not Started Yet", hidden: false });
      const freshId = freshRes.body.id;

      const res = await request(server)
        .post(`/api/v1/encounters/${freshId}/next-turn`)
        .set(dm);
      expect(res.status).toBe(400);
    });
    ...
    it("/end on a non-running (preparing) encounter is rejected 400", async () => {
      ...
      const freshRes = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Never Started", hidden: false });
      const freshId = freshRes.body.id;

      const res = await request(server)
        .post(`/api/v1/encounters/${freshId}/end`)
        .set(dm);
      expect(res.status).toBe(400);
    });
    ...
    it("dm ends the encounter and character hp is written back", async () => {
      ...
      const charRes = await request(server)
        .get(`/api/v1/characters/${ownedCharacterId}`)
        .set(dm);
    ...
    it("/reopen on a non-ended (preparing) encounter is rejected 400", async () => {
      ...
      const freshRes = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Still Preparing", hidden: false });
      const freshId = freshRes.body.id;

      const res = await request(server)
        .post(`/api/v1/encounters/${freshId}/reopen`)
        .set(dm);
      expect(res.status).toBe(400);
    });
    ...
    it("a non-dm cannot reopen an ended encounter (403)", async () => {
      ...
      const server = ctx.app.getHttpServer();
      const res = await request(server)
        .post(`/api/v1/encounters/${encounterId}/reopen`)
        .set(player);
      expect(res.status).toBe(403);
    });
    ...
    it("dm reopens an ended encounter back to running, preserving round and clearing endedAt", async () => {
      ...
      const endAgain = await request(server)
        .post(`/api/v1/encounters/${encounterId}/end`)
        .set(dm);
    ...
    it("delete trashes the encounter but keeps combatants and events for restore (issue #701)", async () => {
      ...
      const encRes = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Cleanup Test", hidden: false });
      expect(encRes.status).toBe(201);
      const encId = encRes.body.id;
      await request(server)
        .post(`/api/v1/encounters/${encId}/roll-initiative`)
        .set(dm);
      const startRes = await request(server)
        .post(`/api/v1/encounters/${encId}/start`)
        .set(dm);
      expect(startRes.status).toBe(201);

      const combatantsBefore = await db
        .select()
        .from(combatantsTable)
        .where(eq(combatantsTable.encounterId, encId));
      const eventsBefore = await db
        .select()
        .from(encounterEventsTable)
        .where(eq(encounterEventsTable.encounterId, encId));
      expect(combatantsBefore.length).toBeGreaterThan(0);
      expect(eventsBefore.length).toBeGreaterThan(0);

      const del = await request(server)
        .delete(`/api/v1/encounters/${encId}`)
        .set(dm);
      expect(del.status).toBe(200);

      const combatantsAfter = await db
        .select()
        .from(combatantsTable)
        .where(eq(combatantsTable.encounterId, encId));
      ...
      const restore = await request(server)
        .post(`/api/v1/encounters/${encId}/restore`)
        .set(dm);
      expect(restore.status).toBe(201);
      expect(
        (await request(server).get(`/api/v1/encounters/${encId}`).set(dm))
          .status,
      ).toBe(200);
    });
  });
  ...
  describe("lowercase stat keys (issue #162)", () => {
    ...
    it("a lowercase-dex character gets the correct DEX-derived initMod on its combatant", async () => {
      ...
      const encRes = await request(server)
        .post(`/api/v1/campaigns/${lcCampaignId}/encounters`)
        .set(dm)
        .send({ name: "Initiative Check", hidden: false });
      expect(encRes.status).toBe(201);
  ...
  describe("ended encounter is immutable (issues #163, #470)", () => {
    ...
    beforeAll(async () => {
      ...
      const campRes = await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Ended Encounter Campaign" });
      ...
      const encRes = await request(server)
        .post(`/api/v1/campaigns/${endedCampaignId}/encounters`)
        .set(dm)
        .send({ name: "Last Stand", hidden: false });
      endedEncounterId = encRes.body.id;
      heroCombatantId = encRes.body.combatants[0].id;

      await request(server)
        .post(`/api/v1/encounters/${endedEncounterId}/roll-initiative`)
        .set(dm);
      const startRes = await request(server)
        .post(`/api/v1/encounters/${endedEncounterId}/start`)
        .set(dm);
      expect(startRes.status).toBe(201);
      const endRes = await request(server)
        .post(`/api/v1/encounters/${endedEncounterId}/end`)
        .set(dm);
    ...
    it("the ended encounter is still viewable", async () => {
      const server = ctx.app.getHttpServer();
      const res = await request(server)
        .get(`/api/v1/encounters/${endedEncounterId}`)
        .set(player);
      expect(res.status).toBe(200);
      expect(res.body.status).toBe("ended");
    });
    ...
    it("patching a combatant on an ended encounter is rejected (409) and does NOT change the character sheet", async () => {
      ...
      const before = await request(server)
        .get(`/api/v1/characters/${heroCharacterId}`)
        .set(dm);
      ...
      const dmPatch = await request(server)
        .patch(
          `/api/v1/encounters/${endedEncounterId}/combatants/${heroCombatantId}`,
        )
        .set(dm)
        .send({ hpSet: 13 });
        ...
        [
          ...
          () =>
            request(server)
              .patch(`/api/v1/encounters/${endedEncounterId}`)
              .set(dm)
              .send({ mapAttachmentId: upload.body.id }),
        ],
        [
          "gridSize",
          () =>
        ...
        [
          ...
          () =>
            request(server)
              .patch(`/api/v1/encounters/${endedEncounterId}`)
              .set(dm)
              .send({ locationId: loc.body.id }),
        ],
        [
          "generate-map",
          () =>
            request(server)
              .post(`/api/v1/encounters/${endedEncounterId}/generate-map`)
              .set(dm)
              .send({ kind: "dungeon", size: "large" }),
        ],
      ] as const;
    ...
    it("after /reopen the same combatant patch succeeds and mirrors to the character sheet again", async () => {
      ...
      const reopen = await request(server)
        .post(`/api/v1/encounters/${endedEncounterId}/reopen`)
        .set(dm);
  ...
  describe("dice rolling", () => {
    ...
    it("a roll audit entry exists", async () => {
      ...
      const rows = await db
        .select()
        .from(auditLog)
        .where(eq(auditLog.action, "dice.roll"));
  ...
  describe("persistent combat log (issue #61)", () => {
    ...
    beforeAll(async () => {
      ...
      const campRes = await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Combat Log Campaign" });
      logCampaignId = campRes.body.id;

      const encRes = await request(server)
        .post(`/api/v1/campaigns/${logCampaignId}/encounters`)
        .set(dm)
        .send({ name: "Logged Fight", hidden: false });
      ...
      await request(server)
        .post(`/api/v1/encounters/${logEncounterId}/roll-initiative`)
        .set(dm);
      const start = await request(server)
        .post(`/api/v1/encounters/${logEncounterId}/start`)
        .set(dm);
    ...
    it("starting the encounter seeds a single opening-turn event", async () => {
      const server = ctx.app.getHttpServer();
      const res = await request(server)
        .get(`/api/v1/encounters/${logEncounterId}/events`)
        .set(dm);
      expect(res.status).toBe(200);
      const turns = (res.body as Array<{ type: string; round: number }>).filter(
    ...
    it("a condition mutation appends a condition event", async () => {
      ...
      const res = await request(server)
        .get(`/api/v1/encounters/${logEncounterId}/events`)
        .set(dm);
      const cond = (res.body as Array<{ type: string; detail: string }>).filter(
        (e) => e.type === "condition",
      );
    ...
    it("a heal mutation appends a heal event", async () => {
      ...
      const res = await request(server)
        .get(`/api/v1/encounters/${logEncounterId}/events`)
        .set(dm);
      const heal = (res.body as Array<{ type: string; detail: string }>).filter(
        (e) => e.type === "heal",
      );
    ...
    it("a next-turn mutation appends a turn event", async () => {
      ...
      const before = (
        await request(server)
          .get(`/api/v1/encounters/${logEncounterId}/events`)
          .set(dm)
      ).body.filter((e: { type: string }) => e.type === "turn").length;

      const turn = await request(server)
        .post(`/api/v1/encounters/${logEncounterId}/next-turn`)
        .set(dm);
      ...
      const res = await request(server)
        .get(`/api/v1/encounters/${logEncounterId}/events`)
        .set(dm);
      const turns = (res.body as Array<{ type: string }>).filter(
        (e) => e.type === "turn",
      );
    ...
    it("a lethal mutation appends a death event for the monster", async () => {
      ...
      const res = await request(server)
        ...
        .set(dm);
      const death = (
        res.body as Array<{ type: string; target: string }>
    ...
    it("the log persists and lists in chronological order across the mutations above", async () => {
      ...
      const types = new Set(
        (res.body as Array<{ type: string }>).map((e) => e.type),
      );
  ...
  describe("combat-log damage attribution (issue #620)", () => {
    ...
    beforeAll(async () => {
      ...
      const campRes = await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Actor Campaign" });
      actorCampId = campRes.body.id;

      const encRes = await request(server)
        .post(`/api/v1/campaigns/${actorCampId}/encounters`)
        .set(dm)
        .send({ name: "Attributed Fight", hidden: false });
      ...
      const start = await request(server)
        .post(`/api/v1/encounters/${actorEncounterId}/start`)
        .set(dm);
      expect(start.status).toBe(201);
    });
    ...
    it("records the current-turn combatant as the actor of a damage event", async () => {
      ...
      const res = await request(server)
        .get(`/api/v1/encounters/${actorEncounterId}/events`)
        .set(dm);
      const damage = (
        res.body as Array<{
          type: string;
    ...
    it("accepts an explicit actorId that overrides the current-turn combatant", async () => {
      ...
      ).filter((e) => e.type === "damage" && e.target === "Ember");
      expect(damage).toHaveLength(1);
    ...
    it('drops the actor when the attacker is the target itself (no "Ember: took 4 damage")', async () => {
      ...
      const selfDamage = (
        ...
        (e) =>
          e.type === "damage" && e.target === "Ember" && e.actor !== "Goblin",
      );
    ...
    it("ignores an actorId that does not reference a combatant in this encounter", async () => {
      ...
      ).filter((e) => e.type === "damage" && e.target === "Goblin");
  ...
  describe("player apply-damage to their own combatant (issue #1478)", () => {
    ...
    async function setupOwnedFight(
      name: string,
      initiative: { hero: number; monster: number },
    ) {
      const server = ctx.app.getHttpServer();
      const campRes = await request(server)
        .post("/api/v1/campaigns")
      ...
      const encRes = await request(server)
        .post(`/api/v1/campaigns/${campId}/encounters`)
        .set(dm)
        .send({ name: "Drill", hidden: false });
      expect(encRes.status).toBe(201);
      const encId = encRes.body.id;
      const heroId = (
        encRes.body.combatants as Array<{
          id: number;
          characterId: number | null;
        }>
      ).find((c) => c.characterId === charRes.body.id)?.id;
      ...
      await request(server)
        ...
        .set(dm)
        .send({ initiative: initiative.hero });
      await request(server)
        .patch(`/api/v1/encounters/${encId}/combatants/${monsterId}`)
        .set(dm)
        .send({ initiative: initiative.monster });
      ...
      expect(start.body.currentCombatantId).toBe(
        initiative.hero > initiative.monster ? heroId : monsterId,
      );

      return {
        encId,
        heroId: heroId as number,
        monsterId: monsterId as number,
      };
    }
    ...
    it("succeeds while a MONSTER holds the turn, attributing the hit to the current-turn combatant", async () => {
      ...
      const { encId, heroId } = await setupOwnedFight(
        "Owned Fight — monster turn",
        { hero: 4, monster: 20 },
      );
      ...
      const res = await request(server)
        .get(`/api/v1/encounters/${encId}/events`)
        .set(dm);
      const damage = (
        res.body as Array<{
          type: string;
    ...
    it("still REJECTS a non-DM patch that carries an explicit actorId (anti-spoofing intact)", async () => {
      ...
      const { encId, heroId, monsterId } = await setupOwnedFight(
        "Owned Fight — spoof attempt",
        { hero: 4, monster: 20 },
      );
      ...
      const after = await request(server)
        .get(`/api/v1/encounters/${encId}`)
        .set(dm);
      expect(
        (
          after.body.combatants as Array<{ id: number; hpCurrent: number }>
        ).find((c) => c.id === heroId)?.hpCurrent,
      ).toBe(45);
      const events = await request(server)
        .get(`/api/v1/encounters/${encId}/events`)
        .set(dm);
      expect(
        (events.body as Array<{ type: string }>).filter(
          (e) => e.type === "damage",
        ),
      ).toHaveLength(0);
    });
  ...
  describe("combatant statblock exposure (issue #56)", () => {
    it("a compendium-linked combatant exposes its statblock (AC / actions / abilities) via the rules read path", async () => {
        ...
        .values({
          ...
          dataJson: JSON.stringify({
            ...
            actions: [
              {
                name: "Bite",
                desc: "+5 to hit, reach 5 ft., one target. 2d6+3 fire damage.",
              },
            ],
          }),
          createdAt: ts,
      ...
      const campRes = await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Statblock Campaign" });
      const sbCampaignId = campRes.body.id;
      const encRes = await request(server)
        .post(`/api/v1/campaigns/${sbCampaignId}/encounters`)
        .set(dm)
        .send({ name: "SB Fight", hidden: false });
      ...
      const entryRes = await request(server)
        .get(`/api/v1/rules/entries/${entry.id}`)
        .set(dm);
...
describe("encounters — optimistic concurrency (issue #532, e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "CAS Encounter Campaign" })
    ).body.id;
    encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Ambush at the Crossroads", hidden: false })
    ).body.id;
  });
  ...
  it("two concurrent updates: the second (stale) one gets 409 and the first edit survives", async () => {
    ...
    const tabA = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    const tabB = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
...
describe("encounters — lifecycle write serialization (issue #1461, e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    const campaign = await request(server)
      .post("/api/v1/campaigns")
      .set(dm)
      .send({ name: "Lifecycle Race Campaign" });
    const encounter = await request(server)
      .post(`/api/v1/campaigns/${campaign.body.id}/encounters`)
      .set(dm)
  ...
  it("concurrent initiative rolls commit one value/log set, then only one concurrent end wins", async () => {
    const server = ctx.app.getHttpServer();
    const firstCombatant = (
      await request(server).get(`/api/v1/encounters/${encounterId}`).set(dm)
    ).body.combatants.find(
      (combatant: { name: string }) => combatant.name === "First",
    );
    expect(
      (
        await request(server)
          .patch(
            `/api/v1/encounters/${encounterId}/combatants/${firstCombatant.id}`,
          )
          .set(dm)
          .send({ initiative: 19 })
      ).status,
    ...
    const [firstRoll, secondRoll] = await Promise.all([
      ...
      request(server)
        .post(`/api/v1/encounters/${encounterId}/roll-initiative`)
        .set({ ...dm, "x-dev-user": "dm-2" })
        .send({}),
    ]);
    expect(
      [firstRoll.body.rolledCount, secondRoll.body.rolledCount].sort(),
    ).toEqual([0, 1]);
    ...
    const rollEvents = (
      await db
        .select()
        .from(encounterEventsTable)
        .where(eq(encounterEventsTable.encounterId, encounterId))
    ...
    expect(
      (
        ...
        (combatant: { id: number }) => combatant.id === firstCombatant.id,
      ).initiative,
    ).toBe(19);
...
describe("encounters (e2e, real cookie sessions — non-member access)", () => {
  ...
  beforeAll(async () => {
    ...
    await adminAgent
      ...
      .send({ username: "root-admin-enc", password: "admin-password-1" });
    await adminAgent
      .post("/api/v1/users")
      .send({
        username: "dm-real-enc",
        password: "password-dm-1",
        serverRole: "user",
      });
    await adminAgent
      .post("/api/v1/users")
      .send({
        username: "outsider-real-enc",
        password: "password-out-1",
        serverRole: "user",
      });
    ...
    await dmAgent
      ...
      .send({ username: "dm-real-enc", password: "password-dm-1" });

    outsiderAgent = request.agent(server);
    await outsiderAgent
      .post("/api/v1/auth/login")
      .send({ username: "outsider-real-enc", password: "password-out-1" });

    const createRes = await dmAgent
      .post("/api/v1/campaigns")
      .send({ name: "Private Encounter Campaign" });
    campaignId = createRes.body.id;

    const encRes = await dmAgent
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .send({ name: "Secret Fight", hidden: false });
...
describe("encounters — issue #51: character uniqueness guard (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    ariaId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/characters`)
        .set(dm)
        .send({ name: "Aria", hpCurrent: 20, hpMax: 20 })
    ).body.id;
  ...
  it("re-adding a character already present in the encounter is rejected 409", async () => {
    ...
    const before = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    const existingAria = (before.body.combatants as CombatantShape[]).find(
      (c) => c.characterId === ariaId,
    )!;
    const res = await request(server)
      .post(`/api/v1/encounters/${encounterId}/combatants`)
      .set(dm)
    ...
    const getRes = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    const ariaRows = (getRes.body.combatants as CombatantShape[]).filter(
      (c) => c.characterId === ariaId,
    );
  ...
  it("a different, not-yet-present character can still be added (201)", async () => {
    ...
    const bramId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/characters`)
        .set(dm)
        .send({ name: "Bram", hpCurrent: 15, hpMax: 15 })
    ).body.id;
    const res = await request(server)
      .post(`/api/v1/encounters/${encounterId}/combatants`)
      .set(dm)
...
describe("encounters — issue #49: identity-based turn pointer (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    m1Id = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "M1", hpMax: 10 })
    ).body.id;
    m2Id = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "M2", hpMax: 10 })
    ).body.id;

    // Deterministic order via explicit initiatives: M1=20, Aria=10, M2=5.
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${m1Id}`)
      .set(dm)
      .send({ initiative: 20 });
    await request(server)
      ...
      .send({ initiative: 10 });
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${m2Id}`)
      .set(dm)
      .send({ initiative: 5 });
  });
  ...
  it("start pins currentCombatantId to the top of the order (M1)", async () => {
    ...
    expect((res.body.combatants as CombatantShape[]).map((c) => c.id)).toEqual([
      m1Id,
      ariaCombatantId,
      m2Id,
    ]);
  ...
  it("removing a combatant that sorts ABOVE the current actor keeps the pointer on Aria (not shifted)", async () => {
    ...
    const del = await request(server)
      .delete(`/api/v1/encounters/${encounterId}/combatants/${m1Id}`)
      .set(dm);
    expect(del.status).toBe(200);
  ...
  it("a combatant added mid-fight does not move the current pointer", async () => {
    ...
    expect(
      (getRes.body.combatants as CombatantShape[]).map((c) => c.id),
    ).toEqual([ariaCombatantId, m2Id, reinf.body.id]);
  });

  it("removing the CURRENT combatant advances the pointer to the next in order (M2)", async () => {
    const server = ctx.app.getHttpServer();
    const del = await request(server)
      .delete(`/api/v1/encounters/${encounterId}/combatants/${ariaCombatantId}`)
      .set(dm);
    expect(del.status).toBe(200);
  ...
  beforeAll(async () => {
    ...
    campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Skip Downed" })
    ).body.id;

    const encRes = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Skip Fight", hidden: false });
    encounterId = encRes.body.id;
    ...
    aliveId = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "Alive", hpMax: 10 })
    ).body.id;
    downedId = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "Downed", hpMax: 10 })
    ).body.id;
    nextAliveId = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "Next", hpMax: 10 })
    ).body.id;

    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${aliveId}`)
      .set(dm)
      .send({ initiative: 20 });
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${downedId}`)
      .set(dm)
      .send({ initiative: 15 });
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${nextAliveId}`)
      .set(dm)
      .send({ initiative: 10 });
...
describe("encounters — issue #528: removeCombatant increments round on turn wrap (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Round Wrap" })
    ).body.id;

    const encRes = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Wrap Fight", hidden: false });
    encounterId = encRes.body.id;
    ...
    c1Id = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "C1", hpMax: 10 })
    ).body.id;
    c2Id = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "C2", hpMax: 10 })
    ).body.id;
    c3Id = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "C3", hpMax: 10 })
    ).body.id;

    // Deterministic order via explicit initiatives: C1=20, C2=10, C3=5.
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${c1Id}`)
      .set(dm)
      .send({ initiative: 20 });
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${c2Id}`)
      .set(dm)
      .send({ initiative: 10 });
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${c3Id}`)
      .set(dm)
      .send({ initiative: 5 });
    ...
    const next1 = await request(server)
      .post(`/api/v1/encounters/${encounterId}/next-turn`)
      .set(dm);
    expect(next1.body.currentCombatantId).toBe(c2Id);
    const next2 = await request(server)
      .post(`/api/v1/encounters/${encounterId}/next-turn`)
      .set(dm);
  ...
  it("removing the last-in-order current combatant wraps to the top AND increments the round (matches advanceTurn)", async () => {
    ...
    const del = await request(server)
      .delete(`/api/v1/encounters/${encounterId}/combatants/${c3Id}`)
      .set(dm);
    expect(del.status).toBe(200);
...
describe("encounters — issue #54: set/roll initiative for a late joiner (e2e)", () => {
  ...
  it("roll-initiative works while running and fills the late joiner (only null values)", async () => {
    ...
    const aria = (res.body.combatants as CombatantShape[]).find(
  ...
  it("dm can set a specific initiative on any combatant while running", async () => {
    ...
    const cultist = (
      await request(server).get(`/api/v1/encounters/${encounterId}`).set(dm)
    ).body.combatants.find((c: CombatantShape) => c.name === "Cultist");
    const res = await request(server)
...
describe("encounters — issue #702: no-op roll-initiative + rolledCount (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "No-op Initiative Campaign" })
    ).body.id;
  ...
  async function countInitiativeAudits(encounterId: number): Promise<number> {
    ...
    const rows = await db
      .select()
      .from(auditLog)
      .where(eq(auditLog.entityId, encounterId));
    return rows.filter((row) => row.action === "encounter.roll_initiative")
      .length;
  }
  ...
  it("a fully-rolled roster is a no-op: rolledCount=0, no audit entry, no SSE broadcast", async () => {
    ...
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "All Rolled", hidden: false })
    ).body.id;
  ...
  it("a partial roster rolls only the missing ones and returns the count", async () => {
    ...
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Partial", hidden: false })
    ).body.id;
    const getRes = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    ...
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${partyId}`)
      .set(dm)
      .send({ initiative: 7 });
    await request(server)
      .post(`/api/v1/encounters/${encounterId}/combatants`)
    ...
    const pinned = (rollRes.body.combatants as CombatantShape[]).find(
      (c) => c.id === partyId,
    );
  ...
  it("reinforcements added mid-fight roll to a non-zero count, then a fully-rolled re-roll is a no-op", async () => {
    ...
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Reinforce", hidden: false })
    ).body.id;
    ...
    const fillRes = await request(server)
      .post(`/api/v1/encounters/${encounterId}/roll-initiative`)
      .set(dm);
    expect(fillRes.status).toBe(201);
    expect(fillRes.body.rolledCount).toBe(1);
    const lateOrc = (fillRes.body.combatants as CombatantShape[]).find(
      (c) => c.name === "Late Orc",
    );
  ...
  it("a zero-combatant encounter rolls nothing and reports rolledCount=0 without auditing", async () => {
    ...
    const emptyCampaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Empty Encounter Campaign" })
    ).body.id;
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${emptyCampaignId}/encounters`)
        .set(dm)
        .send({ name: "Empty", hidden: false })
    ).body.id;
    const auditsBefore = await countInitiativeAudits(encounterId);
    const res = await request(server)
      .post(`/api/v1/encounters/${encounterId}/roll-initiative`)
  ...
  it("starting a truly empty encounter (no party, never added) is rejected 400 and stays preparing", async () => {
    ...
    const campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Empty Roster Campaign" })
    ).body.id;
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "No Combatants", hidden: false })
    ).body.id;
  ...
  it("a campaign whose only characters are dead/retired auto-adds nobody, so Start is rejected 400", async () => {
    ...
    const campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "All Defeated Campaign" })
    ).body.id;
    ...
    await request(server)
      .post(`/api/v1/campaigns/${campaignId}/characters`)
      .set(dm)
      .send({ name: "Fallen Aria", status: "dead" });
    await request(server)
      .post(`/api/v1/campaigns/${campaignId}/characters`)
      .set(dm)
      .send({ name: "Retired Boro", status: "retired" });

    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Nobody Left", hidden: false })
    ).body.id;
    const getBeforeStart = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
  ...
  it("removing every combatant after auto-add empties the roster and Start is rejected 400", async () => {
    ...
    const campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Removed Roster Campaign" })
    ).body.id;
    await request(server)
      .post(`/api/v1/campaigns/${campaignId}/characters`)
      .set(dm)
    ...
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Emptied Out", hidden: false })
    ).body.id;
    const getRes = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    ...
    for (const c of combatants) {
      const del = await request(server)
        .delete(`/api/v1/encounters/${encounterId}/combatants/${c.id}`)
        .set(dm);
      expect(del.status).toBe(200);
    }
  ...
  it("two concurrent Start attempts on a zero-combatant encounter both 400 (no partial/racy running state)", async () => {
    ...
    const campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "Concurrent Empty Start Campaign" })
    ).body.id;
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Race", hidden: false })
    ).body.id;
...
describe("encounters — issue #50: character/combatant HP stay in sync (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    charId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/characters`)
        .set(dm)
        .send({ name: "Aria", hpCurrent: 30, hpMax: 30 })
    ).body.id;
    const encRes = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
  ...
  async function combatantHp(): Promise<{ hpCurrent: number; hpMax: number }> {
    ...
    const c = (
      await request(server).get(`/api/v1/encounters/${encounterId}`).set(dm)
    ).body.combatants.find((x: CombatantShape) => x.id === combatantId);
  ...
  it("healing on the character sheet mirrors into the live combatant row (the fix)", async () => {
    const server = ctx.app.getHttpServer();
    const res = await request(server)
      .post(`/api/v1/characters/${charId}/hp`)
      .set(dm)
      .send({ delta: 5 });
    expect(res.status).toBe(201);
...
describe("encounters — issue #43: monster HP is redacted for non-DM viewers (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    const encRes = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Boss Fight", hidden: false });
    encounterId = encRes.body.id;
    ariaCombatantId = encRes.body.combatants[0].id;
    ...
    const npcEntityId = (
      await request(server)
        ...
        .set(dm)
        .send({ name: "Captain Vex", hidden: false })
    ).body.id;
    npcCombatantId = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "npc", npcId: npcEntityId, hpMax: 100 })
    ).body.id;
    await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${npcCombatantId}`)
      .set(dm)
      .send({ hpSet: 20 });
  });
  ...
  it("DM sees exact NPC HP; a non-DM sees only a band (NPC HP is DM-controlled, like a monster)", async () => {
    ...
    const dmRes = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    const dmNpc = (dmRes.body.combatants as Row[]).find(
      (c) => c.id === npcCombatantId,
    )!;
    ...
    for (const headers of [player, viewer]) {
      const res = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(headers);
      const npc = (res.body.combatants as Row[]).find(
        (c) => c.id === npcCombatantId,
      )!;
  ...
  it("a hidden NPC combatant hides its identity (npcId + name) from non-DMs (#374)", async () => {
    ...
    const dmRes = await request(server)
      ...
      .set(dm);
    const dmC = (
      dmRes.body.combatants as Array<{
        id: number;
        name: string;
        npcId: number | null;
      }>
  ...
  it('a downed monster (0 HP) bands to "down" for a player', async () => {
    ...
    const res = await request(server)
      ...
      .set(player);
    const boss = (
      res.body.combatants as Array<{ id: number; hpBand: string | null }>
    ).find((c) => c.id === monsterId)!;
...
describe("encounters — issue #86: concurrent HP updates are not lost (e2e)", () => {
  ...
  it("20 concurrent hpDelta requests (DM + owning player) all apply — no lost update", async () => {
    ...
    const getRes = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    const aria = (getRes.body.combatants as CombatantShape[]).find(
      (c) => c.id === ariaCombatantId,
    )!;
    expect(aria.hpCurrent).toBe(20);
  ...
  it("concurrent addCombatant calls get distinct sortOrders (no collision)", async () => {
    const adds = Array.from({ length: 6 }, (_, i) =>
      request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: `Add${i}`, hpMax: 10 }),
    );
    ...
    const getRes = await request(server)
      ...
      .set(dm);
    const sortOrders = (
      getRes.body.combatants as Array<{ sortOrder: number }>
    ).map((c) => c.sortOrder);
...
describe("encounters — issue #57: temp HP / death saves / overkill (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    const encRes = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Grave Peril", hidden: false });
    encounterId = encRes.body.id;
    heroCombatantId = encRes.body.combatants[0].id;
  });
  ...
  async function hero(): Promise<HpShape> {
    ...
    const res = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    return (res.body.combatants as HpShape[]).find(
      (c) => c.id === heroCombatantId,
    )!;
  }
  ...
  it("a monster reduced to 0 just goes down — no death saves", async () => {
    ...
    const res = await request(server)
      .patch(`/api/v1/encounters/${encounterId}/combatants/${ogreId}`)
      .set(dm)
      .send({ hpDelta: -999 });
    expect(res.status).toBe(200);
    expect(res.body.hpCurrent).toBe(0);
...
describe("encounters — issue #114: count add + rename (e2e)", () => {
  ...
  it("count=3 adds three distinguishable, auto-numbered goblins", async () => {
    ...
    const getRes = await request(server)
      ...
      .set(dm);
    const goblins = (
      getRes.body.combatants as Array<{
        id: number;
        name: string;
        hpMax: number;
      }>
    ...
    expect(goblins.map((g) => g.name).sort()).toEqual([
      "Goblin 1",
      "Goblin 2",
      "Goblin 3",
    ]);
...
describe("encounters — issue #39: per-encounter battle map + combatant tokens (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    const encRes = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Map Fight", hidden: false });
    encounterId = encRes.body.id;
    charCombatantId = encRes.body.combatants.find(
      (c: CombatantShape) => c.characterId === ownedCharacterId,
    ).id;
  ...
  it("attaching a map keeps its raw URL hidden while the scoped encounter canvas can load it", async () => {
    const server = ctx.app.getHttpServer();
    const raw = await request(server)
      .get(`/api/v1/attachments/${mapAttachmentId}/file`)
      .set(player);
    expect(raw.status).toBe(404);

    const safe = await request(server)
      .get(`/api/v1/encounters/${encounterId}/map`)
      .set(player);
  ...
  it("rejects an attachment id that does not exist in this campaign (400)", async () => {
    ...
    const res = await request(server)
      .patch(`/api/v1/encounters/${encounterId}`)
      .set(dm)
      .send({ mapAttachmentId: otherMap.body.id });
    expect(res.status).toBe(400);
  });
  ...
  it("DM sets a monster token position; it round-trips", async () => {
    ...
    const res = await request(server)
      .patch(
        `/api/v1/encounters/${encounterId}/combatants/${monsterCombatantId}`,
      )
      .set(dm)
  ...
  it("a placed token persists across a fresh GET (issue #260)", async () => {
    ...
    const getRes = await request(server)
      .get(`/api/v1/encounters/${encounterId}`)
      .set(dm);
    const monster = getRes.body.combatants.find(
      (c: CombatantShape) => c.id === monsterCombatantId,
    );
...
describe("encounters — issue #40: VTT grid, token size & fog of war (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    const encRes = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Grid Fight", hidden: false });
    encounterId = encRes.body.id;
    charCombatantId = encRes.body.combatants.find(
      (c: CombatantShape) => c.characterId === ownedCharacterId,
    ).id;
    monsterCombatantId = (
      await request(server)
        .post(`/api/v1/encounters/${encounterId}/combatants`)
        .set(dm)
        .send({ kind: "monster", name: "Ogre", hpMax: 59 })
    ).body.id;
  });
  ...
  describe("fog of war + server-side token redaction", () => {
    beforeAll(async () => {
      ...
      await request(server)
        .patch(
          `/api/v1/encounters/${encounterId}/combatants/${charCombatantId}`,
        )
        .set(dm)
    ...
    it("the DM still sees every token position (no redaction)", async () => {
      ...
      const res = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(dm);
      const monster = res.body.combatants.find(
        (c: CombatantShape) => c.id === monsterCombatantId,
      );
    ...
    it("a player does NOT receive the monster in the dark, but DOES see the token in the revealed region", async () => {
      ...
      const pc = res.body.combatants.find(
        (c: CombatantShape) => c.id === charCombatantId,
      );
    ...
    it("issue #418: owned / allied / enemy tokens crossing fog get tokenHiddenByFog without leaking coords", async () => {
      ...
      await request(server)
        ...
        .set(dm)
        .send({
          fog: { enabled: true, revealed: [{ x: 0, y: 0, w: 5, h: 5 }] },
        });
      await request(server)
        .patch(
          `/api/v1/encounters/${encounterId}/combatants/${charCombatantId}`,
      ...
      await request(server)
        .patch(
          `/api/v1/encounters/${encounterId}/combatants/${allyCombatantId}`,
        )
        .set(dm)
      ...
      const owned = playerView.body.combatants.find(
        (c: CombatantShape) => c.id === charCombatantId,
      );
      const allied = playerView.body.combatants.find(
        (c: CombatantShape) => c.id === allyCombatantId,
      );
      const enemy = playerView.body.combatants.find(
        (c: CombatantShape) => c.id === monsterCombatantId,
      );
      ...
      const afterReveal = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(player);
      const ownedAfter = afterReveal.body.combatants.find(
        (c: CombatantShape) => c.id === charCombatantId,
      );
      const alliedAfter = afterReveal.body.combatants.find(
        (c: CombatantShape) => c.id === allyCombatantId,
      );
      const enemyAfter = afterReveal.body.combatants.find(
        (c: CombatantShape) => c.id === monsterCombatantId,
      );
      ...
      const dmView = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(dm);
      const dmOwned = dmView.body.combatants.find(
        (c: CombatantShape) => c.id === charCombatantId,
      );
    ...
    it("reveal_map_region-style reveal (via PATCH) lights the monster corner for players", async () => {
      ...
      await request(server)
        ...
        .set(dm)
        .send({
          fog: { enabled: true, revealed: [{ x: 0, y: 0, w: 100, h: 100 }] },
        });
      const res = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(player);
    ...
    it("an out-of-range fog rectangle is rejected (400)", async () => {
      ...
      const res = await request(server)
        ...
        .set(dm)
        .send({
          fog: { enabled: true, revealed: [{ x: -5, y: 0, w: 200, h: 10 }] },
        });
      expect(res.status).toBe(400);
    });
    ...
    it("issue #472: per-region fog edits with ids persist for every client read", async () => {
      ...
      const playerRead = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(player);
      expect(playerRead.body.fog.revealed).toEqual([
        { id: "east", x: 50, y: 50, w: 35, h: 35 },
      ]);
    });
  });
  ...
  describe("issue #465: AoE template fog secrecy", () => {
    ...
    it("the DM still receives every AoE template", async () => {
      ...
      const res = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(dm);
      expect(res.body.aoe.map((t: { id: string }) => t.id).sort()).toEqual([
        "dark-circle",
        "lit-cone",
        "player-fireball",
      ]);
    ...
    it("a player does NOT receive AoE templates in unrevealed fog", async () => {
      ...
      const res = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(player);
      expect(res.body.aoe.map((t: { id: string }) => t.id)).toEqual([
        "lit-cone",
        "player-fireball",
      ]);
    });
  ...
  it("returns the encounter without touching updatedAt, audit, or SSE for a semantic no-op", async () => {
    ...
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Stable Grid", hidden: false })
    ).body.id;
    const patch = {
      name: "Stable Grid",
      locationId: null,
    ...
    const beforeAudit = (
      await db.select().from(auditLog).where(eq(auditLog.entityId, encounterId))
    ).filter((row) => row.action === "encounter.update");
    const broadcasts: Array<{ type: string; encounterId?: number }> = [];
    const subscription = ctx.app
      .get(CampaignEventsService)
      .streamFor(campaignId)
    ...
    try {
      ...
      const response = await request(server)
        ...
        .send({
          ...patch,
          fog: { revealed: [{ h: 25, w: 25, y: 0, x: 0 }], enabled: true },
        });
  ...
  it("allows exactly one meaningful default PATCH when two clients race", async () => {
    ...
    const encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Two-client Grid", hidden: false })
    ).body.id;
    expect(
      (
        await request(server)
    ...
    try {
      const [left, right] = await Promise.all([
        request(server)
          .patch(`/api/v1/encounters/${encounterId}`)
          .set(dm)
          .send({ gridScale: 5, gridUnit: "ft" }),
        request(server)
          .patch(`/api/v1/encounters/${encounterId}`)
          .set(dm)
          .send({ gridUnit: "ft", gridScale: 5 }),
      ]);
      ...
      const audits = (
        ...
        (row) =>
          row.action === "encounter.update" && row.detail.includes("gridScale"),
      );
...
describe("encounters — issue #238: hex grid, shared AoE templates & pings (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    campaignId = (
      await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "VTT238 Campaign" })
    ).body.id;
    encounterId = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Hexy Fight", hidden: false })
    ).body.id;
  });
  ...
  it("DM sets hex orientation; it round-trips with default pointy for legacy encounters", async () => {
    ...
    const fresh = (
      await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Hex orient", hidden: false })
    ).body.id;
    expect(
      (await request(server).get(`/api/v1/encounters/${fresh}`).set(dm)).body
        .hexOrientation,
    ).toBe("pointy");
    const res = await request(server)
      .patch(`/api/v1/encounters/${fresh}`)
      .set(dm)
      .send({ gridType: "hex", hexOrientation: "flat" });
    expect(res.status).toBe(200);
    expect(res.body.hexOrientation).toBe("flat");
    expect(
      (await request(server).get(`/api/v1/encounters/${fresh}`).set(player))
        .body.hexOrientation,
    ).toBe("flat");
    const bad = await request(server)
      .patch(`/api/v1/encounters/${fresh}`)
      .set(dm)
      .send({ hexOrientation: "diamond" });
  ...
  it("DM adds shared cone + line + circle AoE templates; they round-trip and persist for players", async () => {
    ...
    expect(res.body.aoe.map((t: { shape: string }) => t.shape).sort()).toEqual([
      "circle",
      "cone",
      "line",
    ]);
  ...
  it("any member (DM or player) may ping the map; a bad coordinate is rejected (400)", async () => {
    ...
    expect(
      (
        await request(server)
          .post(`/api/v1/encounters/${encounterId}/ping`)
          .set(dm)
          .send({ x: 40, y: 60 })
      ).status,
    ).toBe(201);
  ...
  it("a member demoted from player to viewer loses ping authority on the very next request, and no ping event is broadcast (#1636)", async () => {
    ...
    const stillPlayer = await request(server)
      .post(`/api/v1/encounters/${encounterId}/ping`)
      .set(player)
      .send({ x: 51, y: 51 });
...
describe("encounter linking, campaign-summary digest & difficulty (e2e, issues #126 + #58)", () => {
  ...
  beforeAll(async () => {
    ...
    const loc = await request(server)
      ...
      .set(dm)
      .send({ name: "Thornbridge", status: "explored" });
    locationId = loc.body.id;
    const quest = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/quests`)
      .set(dm)
      .send({ title: "The Everflame", hidden: false });
    questId = quest.body.id;
    const session = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/sessions`)
      .set(dm)
      .send({ title: "Session One" });
  ...
  it("create with location/quest/session links round-trips", async () => {
    ...
    const got = await request(server)
      .get(`/api/v1/encounters/${res.body.id}`)
      .set(dm);
  ...
  it("update edits the name and re-attaches / clears links", async () => {
    ...
    const created = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Loose fight", hidden: false });
  ...
  it("rejects a cross-campaign link target with 404", async () => {
    const server = ctx.app.getHttpServer();
    const otherCamp = await request(server)
      .post("/api/v1/campaigns")
      .set(dm)
      .send({ name: "Other" });
    ...
    const enc = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "X", hidden: false });
    const res = await request(server)
      .patch(`/api/v1/encounters/${enc.body.id}`)
      .set(dm)
      .send({ locationId: otherLoc.body.id });
    expect(res.status).toBe(404);
  });
  ...
  it("create rejects foreign location/quest/session links with 404 and writes nothing (issue #864)", async () => {
    ...
    const otherCamp = await request(server)
      .post("/api/v1/campaigns")
      .set(dm)
      .send({ name: "Other Create Links" });
    const otherLoc = await request(server)
      .post(`/api/v1/campaigns/${otherCamp.body.id}/locations`)
      .set(dm)
      .send({ name: "Elsewhere" });
    const otherQuest = await request(server)
      .post(`/api/v1/campaigns/${otherCamp.body.id}/quests`)
      .set(dm)
      .send({ title: "Foreign" });
    const otherSession = await request(server)
      .post(`/api/v1/campaigns/${otherCamp.body.id}/sessions`)
      .set(dm)
      .send({ title: "Foreign Session" });
  ...
  it("get_campaign_summary (REST) now includes an encounters digest", async () => {
    ...
    const digest = summary.body.encounters.find(
      (e: { id: number }) => e.id === enc.body.id,
    );
  ...
  it("digest downCount excludes dead monsters and reports them via monstersDefeated (issue #625)", async () => {
    ...
    await request(server)
      .patch(`/api/v1/encounters/${enc.body.id}/combatants/${mon1.body.id}`)
      .set(dm)
      .send({ hpSet: 0 });
    await request(server)
      .patch(`/api/v1/encounters/${enc.body.id}/combatants/${mon2.body.id}`)
      .set(dm)
      .send({ hpSet: 0 });
    const pcList = await request(server)
      .get(`/api/v1/encounters/${enc.body.id}`)
      .set(dm);
    const firstPc = pcList.body.combatants.find(
      (c: { kind: string }) => c.kind === "character",
    );
    expect(firstPc).toBeDefined();
    await request(server)
      .patch(`/api/v1/encounters/${enc.body.id}/combatants/${firstPc.id}`)
      .set(dm)
      .send({ hpSet: 0 });
  ...
  it("a note can pin to entityType 'encounter'", async () => {
    ...
    const enc = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Noted fight", hidden: false });
    const note = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/notes`)
      .set(dm)
      .send({
        body: "They fled north",
        entityType: "encounter",
        entityId: enc.body.id,
      });
  ...
  describe("encounter link metadata & backlinks (issue #480)", () => {
    it("GET encounter returns resolved link labels for every role", async () => {
      ...
      for (const headers of [dm, player, viewer]) {
        ...
        expect(got.body.locationLink).toEqual({
          id: locationId,
          label: "Thornbridge",
        });
        expect(got.body.questLink).toEqual({
          id: questId,
          label: "The Everflame",
        });
        expect(got.body.sessionLink).toEqual({
          id: sessionId,
          label: "Session One",
        });
      }
    ...
    it("redacts hidden linked entities for non-DM and omits their labels", async () => {
      ...
      const enc = await request(server)
        ...
        .send({
          name: "Secret ambush",
          questId: secretQuest.body.id,
          locationId: secretLoc.body.id,
          hidden: false,
        });

      const dmGot = await request(server)
        .get(`/api/v1/encounters/${enc.body.id}`)
        .set(dm);
      expect(dmGot.body.questLink).toEqual({
        id: secretQuest.body.id,
        label: "Hidden Plot",
      });
      expect(dmGot.body.locationLink).toEqual({
        id: secretLoc.body.id,
        label: "Secret Cave",
      });
    ...
    it("broken links stay unavailable for the DM without leaking a label", async () => {
      ...
      const loc = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/locations`)
        .set(dm)
        .send({ name: "Gone Place", status: "explored" });
      const enc = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Orphan link", locationId: loc.body.id, hidden: false });
      const removed = await request(server)
        .delete(`/api/v1/locations/${loc.body.id}`)
        .set(dm);
    ...
    it("location/quest/session GET surfaces linked encounter backlinks", async () => {
      ...
      expect(loc.body.linkedEncounters).toEqual(
        expect.arrayContaining([
          { id: enc.body.id, name: "Backlink fight", status: "preparing" },
        ]),
      );
      expect(
        loc.body.linkedEncounters.some(
          (e: { id: number }) => e.id === hiddenEnc.body.id,
        ),
      ).toBe(false);
    ...
    it("campaign summary digests include resolved link metadata", async () => {
      ...
      expect(digest).toBeDefined();
      expect(digest.locationLink).toEqual({
        id: locationId,
        label: "Thornbridge",
      });
  ...
  it("difficulty band computes correctly for a known party + monster set", async () => {
    ...
    const enc = await request(server)
      .post(`/api/v1/campaigns/${campaignId}/encounters`)
      .set(dm)
      .send({ name: "Ogre fight", hidden: false });
  ...
  describe("encounter generator (issue #304)", () => {
    ...
    it("generates a deadly group (read-only preview) within the target band using compendium monsters", async () => {
      ...
      expect(
        list.body.every(
          (e: { name: string }) => e.name !== `Generated deadly encounter`,
        ),
      ).toBe(true);
    });
    ...
    it("is reproducible by seed", async () => {
      ...
      const a = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters/generate`)
        .set(dm)
        .send({ difficulty: "medium", seed: 99 });
    ...
    it("honors an explicit party override and CR filters", async () => {
      ...
      expect(
        res.body.combatants.every(
          (c: { ruleEntryId: number }) => c.ruleEntryId === goblinEntryId,
        ),
      ).toBe(true);
    });
    ...
    it("commit=true persists a hidden, preparing encounter with the generated monsters (dm only)", async () => {
      ...
      const monsters = (
        res.body.encounter.combatants as Array<{ kind: string }>
  ...
  describe("encounter preview / tune / idempotent commit (issue #412)", () => {
    ...
    it("tunes deterministically: reroll one slot is reproducible and pin survives reroll-all", async () => {
      ...
      const a = await request(server)
        ...
        .send({
          ...
          seed: base.body.seed,
          roster: plan,
          tune: { op: "reroll-slot", slotId, seed: 321 },
        });
      const b = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters/preview`)
        .set(dm)
      ...
      const pinned = await request(server)
        ...
        .send({
          ...
          seed: base.body.seed,
          roster: plan,
          tune: { op: "pin", slotId, pinned: true },
        });
      const afterAll = await request(server)
        ...
        .send({
          difficulty: "hard",
          seed: pinned.body.seed,
          roster: pinned.body.plan,
          tune: { op: "reroll-all", seed: 9999 },
        });
      const survivor = afterAll.body.plan.find(
        (s: { slotId: string }) => s.slotId === slotId,
      );
    ...
    it("commits a tuned roster atomically and is idempotent by key", async () => {
      ...
      const monsters = (
        first.body.encounter.combatants as Array<{ kind: string }>
      ).filter((c) => c.kind === "monster");
      expect(monsters.length).toBeGreaterThan(0);
      ...
      expect(second.body.encounter.combatants.length).toBe(
        first.body.encounter.combatants.length,
      );
    });
    ...
    it("a non-DM may preview but not commit", async () => {
      ...
      const commit = await request(server)
        ...
        .send({
          idempotencyKey: "nope",
          roster: preview.body.plan.length
            ? preview.body.plan
            : [
                {
                  slotId: "s1",
                  ruleEntryId: previewGoblinId,
                  count: 1,
                  pinned: false,
                  seed: 1,
                },
  ...
  describe("hidden encounter secrecy (issue #262)", () => {
    ...
    it("a hidden encounter is visible to the DM but hidden wholesale from a player/viewer list", async () => {
      ...
      const dmList = await request(server)
        .get(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm);
    ...
    it("a player/viewer GET of a hidden encounter's roster is denied (404); the DM sees it", async () => {
      ...
      const dmGet = await request(server)
        .get(`/api/v1/encounters/${id}`)
        .set(dm);
      expect(dmGet.status).toBe(200);
      expect(
        dmGet.body.combatants.some(
          (c: { kind: string }) => c.kind === "monster",
        ),
      ).toBe(true);
    ...
    it("a player/viewer difficulty read of a hidden encounter is denied (404); the DM sees it", async () => {
      ...
      const dmDiff = await request(server)
        .get(`/api/v1/encounters/${id}/difficulty`)
        .set(dm);
    ...
    it("revealing a hidden encounter (hidden=false) makes its roster + difficulty visible to a player again", async () => {
      ...
      expect(
        (await request(server).get(`/api/v1/encounters/${id}`).set(player))
          .status,
      ).toBe(404);
    ...
    it("a player/viewer events + ping of a hidden encounter are denied (404); the DM can list events (issue #869)", async () => {
      ...
      const dmEvents = await request(server)
        .get(`/api/v1/encounters/${id}/events`)
        .set(dm);
      ...
      for (const who of [player, viewer]) {
        ...
        expect(
          (
            await request(server)
              .post(`/api/v1/encounters/${id}/ping`)
              .set(who)
              .send({ x: 40, y: 60 })
          ).status,
        ).toBe(404);
      ...
      const dmPing = await request(server)
        .post(`/api/v1/encounters/${id}/ping`)
        .set(dm)
        .send({ x: 40, y: 60 });
  ...
  describe("combat-log event secrecy for hidden NPCs (issue #869)", () => {
    ...
    beforeAll(async () => {
      ...
      await request(server)
        .patch(
          `/api/v1/encounters/${secrecyEncounterId}/combatants/${traitorCombatantId}`,
        )
        .set(dm)
        .send({ initiative: 10 });
      ...
      const roster = await request(server)
        .get(`/api/v1/encounters/${secrecyEncounterId}`)
        .set(dm);
      const aria = (
        roster.body.combatants as Array<{
          id: number;
          name: string;
          kind: string;
        }>
      ...
      await request(server)
        .patch(
          `/api/v1/encounters/${secrecyEncounterId}/combatants/${ariaCombatantIdLocal}`,
        )
        .set(dm)
        .send({ initiative: 20 });
      ...
      const start = await request(server)
        .post(`/api/v1/encounters/${secrecyEncounterId}/start`)
        .set(dm);
      expect(start.status).toBe(201);
      ...
      await request(server)
        .post(`/api/v1/encounters/${secrecyEncounterId}/next-turn`)
        .set(dm);
      await request(server)
        .patch(
          `/api/v1/encounters/${secrecyEncounterId}/combatants/${traitorCombatantId}`,
    ...
    it("DM sees raw NPC names + stable combatant ids on every event type", async () => {
      const server = ctx.app.getHttpServer();
      const res = await request(server)
        .get(`/api/v1/encounters/${secrecyEncounterId}/events`)
        .set(dm);
      expect(res.status).toBe(200);
      ...
      const againstTraitor = body.filter(
      ...
      const opening = body.find(
        (e) => e.type === "turn" && e.detail === "Combat started",
      );
    ...
    it("non-DM list masks hidden NPC identity across event types; detail cannot bypass", async () => {
      ...
      for (const who of [player, viewer]) {
        ...
        expect(
          againstTraitor.some((e) => e.targetId === traitorCombatantId),
        ).toBe(true);
      }
    });
    ...
    it("revealing the NPC unmasks historical events on reconnect/poll-style re-list", async () => {
      const server = ctx.app.getHttpServer();
      const npcList = await request(server)
        .get(`/api/v1/campaigns/${secrecyCampId}/npcs`)
        .set(dm);
      const traitorNpc = (
        npcList.body as Array<{ id: number; name: string }>
      ).find((n) => n.name === "The Traitor")!;
      const revealed = await request(server)
        .patch(`/api/v1/npcs/${traitorNpc.id}`)
        .set(dm)
        .send({ hidden: false });
      expect(revealed.status).toBe(200);
      ...
      const res = await request(server)
        ...
        .set(player);
      expect(res.status).toBe(200);
      const againstTraitor = (
        res.body as Array<{ target: string | null; targetId: number | null }>
      ).filter((e) => e.targetId === traitorCombatantId);
      expect(againstTraitor.length).toBeGreaterThan(0);
      expect(againstTraitor.every((e) => e.target === "The Traitor")).toBe(
        true,
      );
    });
  });
  ...
  describe("clearing initiative back to null (issue #715)", () => {
    ...
    it("clearing initiative is audited with the null payload", async () => {
      const db = ctx.app.get<DrizzleDb>(DB);
      const audits = (
        await db.select().from(auditLog).where(eq(auditLog.entityId, goblinAId))
      ).filter(
        (row) =>
          row.action === "encounter.combatant.update" &&
          row.detail.includes("initiative"),
      );
      ...
      const cleared = audits.find((row) =>
        row.detail.includes('"initiative":null'),
      );
    ...
    it("clearing initiative while running re-sorts order and keeps the current-turn pointer stable", async () => {
      ...
      const beforeStart = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(dm);
      const partyIds = (
        beforeStart.body.combatants as Array<{ id: number; kind: string }>
      )
      ...
      for (const c of beforeStart.body.combatants as Array<{ id: number }>) {
        ...
        await request(server)
          .patch(`/api/v1/encounters/${encounterId}/combatants/${c.id}`)
          .set(dm)
          .send({ initiative: target });
      }
      ...
      const getRes = await request(server)
        .get(`/api/v1/encounters/${encounterId}`)
        .set(dm);
      const order = (getRes.body.combatants as Array<{ id: number }>).map(
        (c) => c.id,
      );
      ...
      const nextRes = await request(server)
        .post(`/api/v1/encounters/${encounterId}/next-turn`)
        .set(dm);
    ...
    it("roll-initiative re-fills a cleared initiative (preparing flow parity)", async () => {
      ...
      const res = await request(server)
        .post(`/api/v1/campaigns/${campaignId}/encounters`)
        .set(dm)
        .send({ name: "Clear Initiative 2", hidden: false });
      ...
      await request(server)
        .patch(`/api/v1/encounters/${enc2}/combatants/${wolfId}`)
        .set(dm)
        .send({ initiative: 5 });
      await request(server)
        .patch(`/api/v1/encounters/${enc2}/combatants/${wolfId}`)
      ...
      const rollRes = await request(server)
        .post(`/api/v1/encounters/${enc2}/roll-initiative`)
        .set(dm);
      expect(rollRes.status).toBe(201);
      const wolf = (
        rollRes.body.combatants as Array<{
          id: number;
          initiative: number | null;
        }>
      ).find((c) => c.id === wolfId);
  ...
  describe("one authoritative live fight (issue #744)", () => {
    ...
    async function makePreparedEncounter(name: string): Promise<number> {
      ...
      const roll = await request(server)
        .post(`/api/v1/encounters/${id}/roll-initiative`)
        .set(dm);
    ...
    it("starting a second encounter while one is already running is rejected 409 with the winner name + deep link", async () => {
      ...
      expect(conflict.body.deepLink).toBe(
        `/c/${campaignId}/encounters/${first}`,
      );

      // The loser stayed 'preparing' — its status flip was atomic with the assertion.
      const loserGet = await request(server())
        .get(`/api/v1/encounters/${second}`)
        .set(dm);
    ...
    it("reopening an ended encounter while another is running is rejected 409", async () => {
      ...
      const conflict = await request(server())
        .post(`/api/v1/encounters/${second}/reopen`)
        .set(dm);
      expect(conflict.status).toBe(409);
      expect(conflict.body.code).toBe("ENCOUNTER_ALREADY_RUNNING");
      expect(conflict.body.encounterId).toBe(first);
    ...
    it("ending the active encounter clears the pointer, allowing a new fight to start", async () => {
      ...
      const endRes = await request(server())
        .post(`/api/v1/encounters/${first}/end`)
        .set(dm);
      expect(endRes.status).toBe(201);
    ...
    it("GET /campaigns/:id/encounters?status=running surfaces the authoritative fight first", async () => {
      ...
      const list = await request(server())
        .get(`/api/v1/campaigns/${campaignId}/encounters?status=running`)
        .set(dm);
      expect(list.status).toBe(200);
    ...
    it("a hidden encounter can still be the authoritative live fight and is the 409 winner by name", async () => {
      ...
      const roll = await request(server)
        .post(`/api/v1/encounters/${hiddenId}/roll-initiative`)
        .set(dm);
      expect(roll.status).toBe(201);

      const started = await request(server)
        .post(`/api/v1/encounters/${hiddenId}/start`)
        .set(dm);
      ...
      const playerList = await request(server)
        .get(`/api/v1/campaigns/${campaignId}/encounters?status=running`)
        .set(player);
      expect(
        playerList.body.some((e: { id: number }) => e.id === hiddenId),
      ).toBe(false);
  ...
  describe("encounter list filter/sort/search (issue #490)", () => {
    ...
    beforeAll(async () => {
      ...
      const campRes = await request(server)
        .post("/api/v1/campaigns")
        .set(dm)
        .send({ name: "List Filter Campaign" });
      ...
      const roll = await request(server)
        .post(`/api/v1/encounters/${runningId}/roll-initiative`)
        .set(dm);
      expect(roll.status).toBe(201);
      const start = await request(server)
        .post(`/api/v1/encounters/${runningId}/start`)
        .set(dm);
      expect(start.status).toBe(201);
      ...
      await request(server)
        .post(`/api/v1/encounters/${endedGoblinId}/roll-initiative`)
        .set(dm);
      await request(server)
        .post(`/api/v1/encounters/${endedGoblinId}/start`)
        .set(dm);
      await request(server)
        .post(`/api/v1/encounters/${endedGoblinId}/end`)
        .set(dm);
    });
    ...
    it("filters by status", async () => {
      ...
      expect(
        preparing.body.every(
          (e: { status: string }) => e.status === "preparing",
        ),
      ).toBe(true);
      expect(
        preparing.body.some((e: { id: number }) => e.id === preparingId),
      ).toBe(true);
      expect(
        preparing.body.some((e: { id: number }) => e.id === runningId),
      ).toBe(false);
    });
...
describe("encounters — issue #487: player end-turn + ready/delay (e2e)", () => {
  ...
  async function seedRunningFight(): Promise<{
    encounterId: number;
    ariaId: number;
    monsterId: number;
  }> {
