...
// A source author can share the importer's label; only the explicit marker distinguishes it.
const IMPORTED_AUTHOR = OLD_LABEL;
...
describe('account historical attribution privacy (e2e)', () => {
  ...
  function retainedRows(db: DrizzleDb) {
    ...
    return {
      ...
      boundaries: db
        ...
        .where(eq(sessionZeroBoundarySubmissions.submitterUserId, stableUserId))
        .all()
        .filter((row) => !row.anonymous),
      anonymousBoundaries: db
        .select()
        .from(sessionZeroBoundarySubmissions)
        .where(eq(sessionZeroBoundarySubmissions.submitterUserId, stableUserId))
        .all()
  ...
  function expectRetainedAttribution(
    db: DrizzleDb,
    label: string,
    options: { expectSupportPreferences: boolean; expectNeutralNotificationCopy: boolean } = {
      expectSupportPreferences: true,
      expectNeutralNotificationCopy: false,
    },
  ) {
    ...
    expect(rows.anonymousBoundaries).not.toHaveLength(0);
    ...
    expect(rows.anonymousBoundaries.every((row) => row.submitterName === '')).toBe(true);
  ...
  beforeAll(async () => {
    ...
    db.insert(sessionZeroBoundarySubmissions).values({ campaignId, kind: 'line', text: 'An anonymous boundary', anonymous: true, submitterUserId: String(memberId), submitterName: '', createdAt: ts, updatedAt: ts }).run();
    ...
    const redactedOriginal = { ...evidencePayload, content: 'Original redacted evidence' };
    ...
    // their stale public labels.
    const staleNoteId = db.insert(notes).values({ campaignId, authorUserId: String(memberId), authorName: STALE_LOCAL_LABEL, authorImported: false, kind: 'note', visibility: 'party_shared', entityType: 'campaign', entityId: campaignId, body: 'Stale local note', resolved: false, resolvedNote: '', deletedAt: null, quarantinedAt: null, quarantinedBy: null, createdAt: ts, updatedAt: ts }).returning().get().id;
    db.insert(comments).values({ campaignId, entityType: 'session', entityId: session.body.id, parentId: null, authorUserId: String(memberId), authorName: STALE_LOCAL_LABEL, authorImported: false, body: 'Stale local comment', inCharacter: false, characterId: null, characterName: null, characterAvatarUrl: null, deletedAt: null, deletedBy: null, editedAt: null, editedBy: null, quarantinedAt: null, quarantinedBy: null, createdAt: ts, updatedAt: ts }).run();
    db.insert(entityRevisions).values({ campaignId, entityType: 'note', entityId: staleNoteId, snapshot: JSON.stringify({ body: 'Stale local note' }), authorUserId: String(memberId), authorName: STALE_LOCAL_LABEL, authorImported: false, authorSource: 'human', authorSourceDetail: '', createdAt: ts, replacedByUserId: String(memberId), replacedByName: STALE_LOCAL_LABEL, replacedByImported: false, replacedBySource: 'human', replacedBySourceDetail: '', replacedAt: ts, restoredFromRevisionId: null, authorshipKnown: true }).run();
    const staleScheduleId = db.insert(scheduledSessions).values({ campaignId, scheduledAt: '2099-06-15T19:30:00.000Z', durationMinutes: 240, title: 'Stale local schedule', location: '', notes: '', status: 'scheduled', cancelledAt: null, cancelledBy: null, cancellationReason: '', sessionId: null, seriesId: null, occurrenceIndex: 0, timezone: '', localStart: '', venueId: null, roomId: null, assignedDmUserId: '', capacity: 0, eventId: '', seasonId: '', icsUid: '', icsSequence: 0, originalScheduledAt: null, createdAt: ts, updatedAt: ts }).returning().get().id;
    db.insert(sessionRsvps).values({ scheduledSessionId: staleScheduleId, userId: String(memberId), userName: STALE_LOCAL_LABEL, userImported: false, status: 'yes', note: 'Stale local RSVP', createdAt: ts, updatedAt: ts }).run();
    ...
    // source provenance.
    importedNoteId = db.insert(notes).values({ campaignId, authorUserId: String(memberId), authorName: IMPORTED_AUTHOR, authorImported: true, kind: 'note', visibility: 'party_shared', entityType: 'campaign', entityId: campaignId, body: 'Imported note', resolved: false, resolvedNote: '', deletedAt: null, quarantinedAt: null, quarantinedBy: null, createdAt: ts, updatedAt: ts }).returning().get().id;
    importedCommentId = db.insert(comments).values({ campaignId, entityType: 'session', entityId: session.body.id, parentId: null, authorUserId: String(memberId), authorName: IMPORTED_AUTHOR, authorImported: true, body: 'Imported comment', inCharacter: false, characterId: null, characterName: null, characterAvatarUrl: null, deletedAt: null, deletedBy: null, editedAt: null, editedBy: null, quarantinedAt: null, quarantinedBy: null, createdAt: ts, updatedAt: ts }).returning().get().id;
    importedRevisionId = db.insert(entityRevisions).values({ campaignId, entityType: 'note', entityId: importedNoteId, snapshot: JSON.stringify({ body: 'Imported note' }), authorUserId: String(memberId), authorName: IMPORTED_AUTHOR, authorImported: true, authorSource: 'human', authorSourceDetail: '', createdAt: ts, replacedByUserId: String(memberId), replacedByName: IMPORTED_AUTHOR, replacedByImported: true, replacedBySource: 'human', replacedBySourceDetail: '', replacedAt: ts, restoredFromRevisionId: null, authorshipKnown: true }).returning().get().id;
    ...
    importedRsvpId = db.insert(sessionRsvps).values({ scheduledSessionId: importedScheduleId, userId: String(memberId), userName: IMPORTED_AUTHOR, userImported: true, status: 'yes', note: 'Imported RSVP', createdAt: ts, updatedAt: ts }).returning().get().id;
  });
  ...
  it('pseudonymizes mutable retained labels on deletion without rewriting audit identity', async () => {
    ...
    const db = ctx.app.get<DrizzleDb>(DB);
    expectRetainedAttribution(db, DELETED_LABEL, {
