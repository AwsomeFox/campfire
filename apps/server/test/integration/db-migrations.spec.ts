  ...
  it('0131 adds the AI-driver grant columns on a legacy control-state table (#1042)', () => {
    ...
    try {
      ...
      try {
        ...
        // #1495 — the aftermath economy-grant budget column, same reasoning as the three above.
        seeded.sqlite.prepare('DELETE FROM __migrations WHERE name = ?').run('0149_ai_driver_aftermath_grant_window_1495');
      } finally {
        seeded.sqlite.close();
      }
