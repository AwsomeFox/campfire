...
describe('encounter token batch atomicity (real SQLite, service layer)', () => {
  ...
  function build() {
    ...
    return { orm, service: new EncountersService(orm, audit, events, rolls, revisions, attachments, new CampaignLibraryService(orm, audit), { notifyCampaign: jest.fn(), notifyUser: jest.fn() } as any) };
  }
