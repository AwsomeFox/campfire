...
describe('encounter token batch atomicity (real SQLite, service layer)', () => {
  ...
  function build() {
    ...
    return { orm, service: new EncountersService(orm, audit, events, rolls, revisions, attachments, new CampaignLibraryService(orm, audit, events), { notifyCampaign: jest.fn().mockResolvedValue(undefined), notifyUser: jest.fn().mockResolvedValue(undefined) } as any) };
  }
