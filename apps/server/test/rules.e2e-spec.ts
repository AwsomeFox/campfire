...
describe('rules / rule packs (e2e, fake Open5e server)', () => {
  ...
  it('install from fake Open5e server -> packs list -> search -> entry fetch -> uninstall', async () => {
    ...
    expect(installRes.body.entryCount).toBe(2 + 2 + 1 + 4 + 2 + 2 + 1); // spells + creatures + magicitems + conditions + classes + species + feats from the fake server
    ...
    expect(reinstallRes.body.skippedExisting).toBe(2 + 2 + 1 + 4 + 2 + 2 + 1);
    expect(reinstallRes.body.entryCount).toBe(2 + 2 + 1 + 4 + 2 + 2 + 1); // unchanged
...
describe('rules / rule packs — concurrent install race (e2e, fake Open5e server)', () => {
  ...
  it('4 concurrent installs: one 201, the rest clean 200s, never a 500', async () => {
    ...
    expect(listRes.body).toHaveLength(1);
    expect(listRes.body[0].entryCount).toBe(2 + 2 + 1 + 4 + 2 + 2 + 1);
