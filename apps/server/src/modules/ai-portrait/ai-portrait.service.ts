...
export class AiPortraitService {
  ...
  async createJob(
    ...
    // to the CALLER (user id) as well as the campaign, so two different members or PATs submitting
    // the same user-chosen key do NOT share a job — the response carries the private prompt and
    // generated bytes, and the job id lets a caller poll/refine/cancel/attach (issue #1321 review).
    const idemKey = opts.idempotencyKey ? `${campaignId}:${auditActor(user)}:${opts.idempotencyKey}` : undefined;
    if (idemKey) {
  ...
  async attach(
    ...
    // this should not throw on authority — but roll back the attachment on ANY linkage failure
    // (including a provenance-audit failure) so a rejected target never leaves a quota-charged
    // orphan (issue #1321 review feedback).
