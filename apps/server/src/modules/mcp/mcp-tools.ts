...
export class McpToolsService {
  ...
  private registerReadTools(server: McpServer, user: RequestUser): void {
    ...
    this.tool(server, 'create_campaign_homebrew', 'Create campaign homebrew, or submit it for DM review with propose=true.', { campaignId: CampaignIdArg, entry: HomebrewRuleEntryInput, propose: z.boolean().optional() }, async ({ campaignId, entry, propose }) => { const role = await this.access.requireMember(user, campaignId as number, { write: true }); if (propose || role !== 'dm') return this.proposalRecords.create(campaignId as number, 'rule_entry', null, 'create', entry as Record<string, unknown>, user, role); return this.rules.createCampaignHomebrew(campaignId as number, entry, user); });
    this.tool(server, 'update_campaign_homebrew', 'Update campaign homebrew, or submit an edit for DM review with propose=true.', { campaignId: CampaignIdArg, entryId: Id, patch: HomebrewRuleEntryUpdate, propose: z.boolean().optional() }, async ({ campaignId, entryId, patch, propose }) => { const role = await this.access.requireMember(user, campaignId as number, { write: true }); if (propose || role !== 'dm') return this.proposalRecords.create(campaignId as number, 'rule_entry', entryId as number, 'update', patch as Record<string, unknown>, user, role); return this.rules.updateCampaignHomebrew(campaignId as number, entryId as number, patch as Record<string, unknown>, user); });
