...
export class AuditService implements OnApplicationBootstrap {
   ...
   */
  async listForCampaignExport(campaignId: number, includePayload = true, includeDetail = true): Promise<{
    ...
    if (snapshotMaxId > 0) {
      ...
      while (true) {
        ...
        entries.push(...page.map((row) => toAuditEntry(row, { includePayload, includeDetail })));
