import { Body, Controller, Delete, Get, HttpCode, Param, ParseIntPipe, Post, Res } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse } from '@nestjs/swagger';
import { Throttle } from '@nestjs/throttler';
import type { Response } from 'express';
import type { CampaignInvite, InvitePreview, Me } from '@campfire/schema';
...
/** Mirrors auth.controller.ts cookieOptions() — the accept flow issues the same session cookie as login. */
...
export class CampaignInvitesController {
  ...
  @Post()
  @ApiOperation({ summary: 'Create an invite link', description: 'dm role required. Role is capped to player/viewer; the code always expires (default 7 days) and may be use-capped.' })
  ...
  @Delete(':inviteId')
  @HttpCode(204)
  @ApiOperation({ summary: 'Revoke an invite link', description: 'dm role required. The code stops working immediately; existing members are unaffected.' })
...
export class JoinController {
  ...
  @INVITE_THROTTLE
  @Get(':code')
  @ApiOperation({ summary: 'Preview an invite', description: 'Unauthenticated. Resolves a join code to the campaign name + role it grants. Unknown, expired, exhausted and revoked codes all return the same 404.' })
