...
export const DRIVER_LIVE_PLAY_TOOL_ARG_RULES: Readonly<Record<string, DriverLivePlayToolArgRule>> = {
  ...
  'update_inventory_item': {
    ...
    // Issue #1901 rework: `displaceEquipped` only does anything alongside `equipped`/
    // `equipSlot`, which are already forbidden here (#1326) — the driver does not touch
    // equip state at all, full stop, so a request to atomically displace another item is
    // exactly as out of bounds as the equip it would ride along with. Forbidding it (not
    // silently allowing) keeps a new schema field from becoming an un-vetted driver
    ...
    forbidden: new Set(['qty', 'ownerType', 'characterId', 'expectedUpdatedAt', 'equipped', 'equipSlot', 'equippedAction', 'displaceEquipped']),
  },
};
