...
export class CampaignCharactersController {
  ...
  @ApiOperation({
    ...
    description:
      'player role required. Reads a PUBLIC D&D Beyond character sheet (unofficial, read-only — no auth, no private data) and creates a Campfire character from it, including attacks, spells, and spell slots (issue #1903). Body is `{ ddbId }` (the numeric character id) or `{ url }` (a character/share link, e.g. https://www.dndbeyond.com/characters/12345678). The sheet must have its privacy set to Public on D&D Beyond. Ownership follows the normal create rules (a player imports for themselves; a dm imports DM-managed). Only available for D&D 5e campaigns (issue #714): a DDB sheet is a 5e character, so the import is rejected with 400 for any other (or no) rule system rather than silently producing a character whose numbers belong to another game. Response is `{ character, summary }`: the created character plus counts of imported actions/spells/slots and the names of any entries that came in text-only (unparseable, never silently dropped).',
  })
  @ApiResponse({ status: 201, description: 'Created character imported from D&D Beyond, with an import summary.' })
