...
const MIGRATIONS: ReadonlyArray<{ name: string; run: (sqlite: Database.Database) => void }> = [
  ...
  // 0110 and 0111 are claimed by in-flight branches (#504/#604 and #501); this
  // migration takes the first ordinal none of them hold, so their merge order
  // cannot renumber it out from under an already-migrated database.
