  ...
  try {
    ...
    await json<ScheduledSessionWithRsvps>(
      await writer.request.post(`/api/v1/campaigns/${secondCampaign.id}/schedule`, {
        ...
        data: { scheduledAt: new Date(Date.now() + 86400000 * 60).toISOString(), title: 'E2E790 Other campaign' },
      }),
    ...
    const created = await json<ScheduledSessionWithRsvps>(
      await writer.request.post(`/api/v1/campaigns/${campaignId}/schedule`, {
        data: { scheduledAt: new Date(Date.now() + 86400000 * 10).toISOString(), title: 'E2E790 Alpha' },
      }),
    ...
    await json<ScheduledSessionWithRsvps>(
      await writer.request.patch(`/api/v1/schedule/${created.id}`, {
        data: { scheduledAt: new Date(Date.now() + 86400000 * 15).toISOString(), title: 'E2E790 Beta', location: 'Replacement table' },
      }),
      'reschedule remotely',
    );
    ...
    await json<ScheduledSessionWithRsvps>(
      await writer.request.patch(`/api/v1/schedule/${created.id}`, {
        data: { scheduledAt: new Date(Date.now() + 86400000 * 20).toISOString(), title: 'E2E790 Gamma', location: 'Remote table' },
      }),
