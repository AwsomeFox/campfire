...
type MotionSample = { animationName: string; transitionDuration: string };
...
test.describe('reduced-motion global policy (issue #594)', () => {
  ...
  test('zeros real HP-flash, dice-roll, ready/level-up, and player-display animations while keeping feedback', async ({
    ...
    const hpBar = page.locator('.cf-hp').first();
    await expect(hpBar).toBeVisible();
    expectNoMotion(await sampleMotion(hpBar), 'character sheet .cf-hp (damage pulse)');
    expectNoMotion(await sampleMotion(hpBar.locator('> div').first()), 'character sheet .cf-hp fill (damage flash)');
    ...
    // (xp is set far past the level-2 threshold in beforeAll), then a real
