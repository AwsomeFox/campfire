...
test('posts as an owned character from the keyboard and renders mobile-safe accessible attribution', async ({ page }) => {
  ...
  if (thread.ok()) {
    const threadPage = (await thread.json()) as { items: Array<{ root: { id: number; body: string }; replies: Array<{ id: number; body: string }> }> };
    const comments = threadPage.items.flatMap((item) => [item.root, ...item.replies]);
...
test('removes a deleted speaker from the owned roster without disabling other owned speakers', async ({ page }) => {
  const { campaignId, navigation } = seed();
  ...
  // The global navigation fixture is also player-owned so player sheet tests stay
  // authorized, therefore a removed temporary speaker must disappear without
  // disabling in-character posting for that remaining owned character.
  expect((await page.request.delete(`/api/v1/characters/${character.id}`)).ok()).toBe(true);
  await page.reload();
  await expect(discussion).toBeVisible();
  await expect(inCharacter).toBeEnabled();
  await inCharacter.check();
  const refreshedSpeakerSelect = discussion.getByRole('combobox', { name: /Speaking as/ }).last();
  await expect(refreshedSpeakerSelect.locator(`option[value="${character.id}"]`)).toHaveCount(0);
  await expect(refreshedSpeakerSelect).not.toHaveValue(String(character.id));
});
