...
test.describe('DM navigation', () => {
  ...
  test('DM sees the Manage section and role badge', async ({ page }) => {
  ...
  // the Manage nav group (sidebar + mobile More sheet share the same builder) so
  // soft-deletes stay findable after the undo snackbar expires. Match by href so
  // a non-empty count badge in the accessible name ("Trash 2") cannot break the assertion.
