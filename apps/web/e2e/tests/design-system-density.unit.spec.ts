...
const DRIFT_PATTERNS: ReadonlyArray<{ name: string; pattern: RegExp; why: string }> = [
  ...
  },
  {
    ...
    // that contain both bare `btn` (excluding `cf-btn` via the negative lookbehind) and
    // `!min-h-0` regardless of token ordering (issue #1695). Ties into #1683's retirement
    // of !min-h-0/!py-* on .btn: the codebase expresses the sub-default floor through the
    // xs density step rather than a hand-pinned value.
    pattern: /\bclass(?:Name)?=\{?["'`][^"'`}\n]*(?<!cf-)\bbtn\b[^"'`}\n]*!min-h-0|\bclass(?:Name)?=\{?["'`][^"'`}\n]*!min-h-0[^"'`}\n]*(?<!cf-)\bbtn\b/,
