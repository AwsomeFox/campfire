...
test.describe('encounter accessibility (#476)', () => {
  ...
  test('map toolbar uses toolbar semantics, aria-pressed, and focus rings', () => {
    expect(battleMap).toMatch(/data-testid="map-toolbar"[\s\S]*role="toolbar"/);
    expect(battleMap).toMatch(/aria-label="Map tools"/);
    expect(battleMap).toMatch(/aria-pressed=\{tool === value\}/);
    expect(battleMap).toMatch(/className="cf-map-tool cf-map-focusable"/);
