...
const SCHEDULE_CONFLICT_FIELDS: Array<ConflictField<ScheduleDraft>> = [
  ...
  { key: 'location', label: 'Location', merge: true },
...
function ScheduleForm({
  ...
  function applyDraft(value: ScheduleDraft) {
    setWhen(toLocalInputValue(value.scheduledAt));
  ...
  async function save() {
    ...
    try {
      ...
      await onSubmit({ ...mine, expectedUpdatedAt: expectedUpdatedAt ?? undefined });
    } catch (err) {
      if (initial && isStaleWrite(err)) {
        try {
          ...
          setConflict({ base: base ?? mine, theirs });
          setBase(theirs);
      ...
      } else setError("Couldn't save the session.");
      setSaving(false);
    }
  }
    ...
    <Card className="space-y-3">
      ...
      {conflict && (() => {
        ...
        return (
          <StaleWriteConflict
            ...
            onResolve={(key, value) => applyDraft({ ...mine, [key]: value } as ScheduleDraft)}
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Btn className="!min-h-0 !py-1.5 text-xs" onClick={save} disabled={saving}>
          {saving ? 'Saving…' : conflict ? 'Save resolution' : initial ? 'Save' : 'Schedule'}
        </Btn>
      </div>
    </Card>
  );
