...
import type { ServerRole, User, ServerSettings } from '@campfire/schema';
...
function SettingsCard({ settings, onChange }: { settings: ServerSettings | null; onChange: () => void }) {
  ...
  async function toggle() {
    ...
    setError(null);
    try {
      await api.patch(`${API}/settings`, { allowLocalLogin: !settings.allowLocalLogin });
      onChange();
    } catch (err) {
  ...
  return (
    <Card className="space-y-3">
      ...
      <div className="cf-inset p-3.5 flex items-center justify-between gap-3 flex-wrap">
        <div>
          <p className="text-sm font-semibold text-white">Allow username/password sign-in for non-admin users</p>
          <p className="text-[11px] text-slate-500">
            When off, only server admins can sign in locally (SSO coming later).
          </p>
        </div>
        <button
          type="button"
          role="switch"
          aria-checked={settings?.allowLocalLogin ?? false}
          onClick={toggle}
          disabled={!settings || saving}
          className="cf-btn !min-h-0 !py-1.5 text-xs"
        >
          {settings?.allowLocalLogin ? 'On' : 'Off'}
        </button>
      </div>
    </Card>
  );
...
function TokensCard() {
  return (
    <Card className="space-y-3 opacity-70">
      <div className="flex items-center justify-between border-b border-slate-700 pb-3">
        <h2 className="font-bold text-white text-sm">
          API tokens <span className="text-slate-500 font-normal">(REST + MCP)</span>
        </h2>
        <Btn className="!min-h-0 !py-1.5 text-xs" disabled>
          + New token
        </Btn>
      </div>
      <p className="text-xs text-slate-500">PAT tokens arrive with the MCP milestone.</p>
    </Card>
  );
}
  ...
  return (
    <Card className="space-y-3 opacity-70">
      ...
      <div className="flex gap-2">
        <Btn ghost className="!min-h-0 !py-2 text-xs" disabled>
          ⬇ JSON export
        </Btn>
        <Btn ghost className="!min-h-0 !py-2 text-xs" disabled>
          ⬇ Markdown zip
