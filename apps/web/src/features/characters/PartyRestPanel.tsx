...
export function PartyRestPanel({ campaignId, characters, onClose, onApplied }: { campaignId: number; characters: Character[]; onClose: () => void; onApplied: () => void }) {
  ...
  return <Card className="space-y-3" aria-labelledby="party-rest-title">
    ...
    {!preview ? <Btn type="button" disabled={busy || ids.length === 0 || (kind === 'custom' && customKeys.length === 0)} onClick={() => void makePreview()}>Preview recovery</Btn> : <><div aria-live="polite">{preview.failures.map((f) => <p key={f.characterName}>{f.detail}</p>)}{preview.characters.map((p) => <div key={p.characterId}><strong>{p.name}</strong>: HP {p.hp.before} → {p.hp.after}; slots {Object.keys(p.spellSlots).join(', ') || 'unchanged'}; resources {Object.keys(p.resources).join(', ') || 'unchanged'}; cleared {p.conditionsCleared.join(', ') || 'none'}; kept {p.conditionsKept.join(', ') || 'none'}.</div>)}</div><label className="block"><input type="checkbox" checked={combatAck} onChange={(e) => setCombatAck(e.target.checked)} /> I acknowledge any running combatants will be synchronized.</label><Btn type="button" disabled={busy || preview.failures.length > 0} onClick={() => void apply()}>Apply recovery</Btn></>}
