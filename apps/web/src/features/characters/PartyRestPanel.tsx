...
export function PartyRestPanel({ campaignId, characters, onClose, onApplied }: { campaignId: number; characters: Character[]; onClose: () => void; onApplied: () => void }) {
  ...
  async function makePreview() {
    ...
    try { setPreview(await api.post<Preview>(`${API}/campaigns/${campaignId}/characters/rest/preview`, kind === 'short' ? { kind, characterIds: ids, perCharacter: shortOptions } : kind === 'custom' ? { kind, characterIds: ids, customResourceKeys: customKeys } : { kind, characterIds: ids })); }
  ...
  return <Card className="space-y-3" aria-labelledby="party-rest-title">
    ...
    {kind === 'custom' && <fieldset><legend className="text-sm font-medium">Resources to restore</legend>{[...new Set(characters.flatMap((c) => Object.keys(c.resources)))].map((key) => <label key={key} className="mr-3"><input type="checkbox" checked={customKeys.includes(key)} onChange={() => { setCustomKeys((old) => old.includes(key) ? old.filter((v) => v !== key) : [...old, key]); setPreview(null); }} /> {key}</label>)}</fieldset>}
    {kind === 'short' && <fieldset><legend className="text-sm font-medium">Hit dice</legend>{characters.filter((c) => selected.has(c.id)).map((c) => <div key={c.id}><label>{c.name} dice <input aria-label={`${c.name} hit dice`} type="number" min="0" value={shortOptions[c.id]?.spendHitDice ?? 0} onChange={(e) => { setShortOptions((old) => ({ ...old, [c.id]: { ...old[c.id], spendHitDice: Number(e.target.value) } })); setPreview(null); }} /></label><label> die size <input aria-label={`${c.name} hit die size`} type="number" min="2" value={shortOptions[c.id]?.hitDie ?? ''} onChange={(e) => { setShortOptions((old) => ({ ...old, [c.id]: { ...old[c.id], hitDie: e.target.value ? Number(e.target.value) : undefined } })); setPreview(null); }} /></label></div>)}</fieldset>}
    <fieldset><legend className="text-sm font-medium">Participants</legend><label className="block"><input type="checkbox" checked={selected.size === characters.filter((c) => c.deathState !== 'dead').length} onChange={(e) => { setSelected(e.target.checked ? new Set(characters.filter((c) => c.deathState !== 'dead').map((c) => c.id)) : new Set()); setPreview(null); }} /> Select eligible</label>{characters.map((c) => <label key={c.id} className="block"><input type="checkbox" disabled={c.deathState === 'dead'} checked={selected.has(c.id)} onChange={() => { setSelected((old) => { const next = new Set(old); next.has(c.id) ? next.delete(c.id) : next.add(c.id); return next; }); setPreview(null); }} /> {c.name}{c.deathState === 'dead' ? ' (dead — unavailable)' : c.status !== 'active' ? ` (${c.status})` : ''}</label>)}</fieldset>
    {!applied && (!preview ? <Btn type="button" disabled={busy || ids.length === 0 || (kind === 'custom' && customKeys.length === 0)} onClick={() => void makePreview()}>Preview recovery</Btn> : <><div aria-live="polite">{preview.failures.map((f) => <p key={f.characterName}>{f.detail}</p>)}{preview.characters.map((p) => <div key={p.characterId}><strong>{p.name}</strong>: HP {p.hp.before} → {p.hp.after} (temp {p.hp.tempBefore} → {p.hp.tempAfter}); death {p.deathState.before} → {p.deathState.after}; hit dice {p.hitDiceSpent} [{p.hitDiceRolls.join(', ')}]; slots {Object.entries(p.spellSlots).map(([k, v]) => `${k}: ${v.before}→${v.after}`).join(', ') || 'unchanged'}; resources {Object.entries(p.resources).map(([k, v]) => `${k}: ${v.before}→${v.after}`).join(', ') || 'unchanged'}.</div>)}</div>{preview.runningCombatantCharacterIds.length > 0 && <label className="block"><input type="checkbox" checked={combatAck} onChange={(e) => setCombatAck(e.target.checked)} /> I acknowledge running combatants will be synchronized.</label>}<Btn type="button" disabled={busy || preview.failures.length > 0 || (preview.runningCombatantCharacterIds.length > 0 && !combatAck)} onClick={() => void apply()}>Apply recovery</Btn></>)}
    {applied && <Btn type="button" disabled={busy} onClick={() => { setBusy(true); void api.post(`${API}/campaigns/${campaignId}/characters/rest/${applied.batchId}/undo`, { idempotencyKey: applied.key }).then(() => { setApplied(null); onApplied(); }).catch((err) => setError(err instanceof ApiError ? err.message : 'Could not undo recovery.')).finally(() => setBusy(false)); }}>Undo recovery</Btn>}
  </Card>;
}
