  ...
  return (
    <Card className="space-y-4" data-testid="generate-encounter-wizard">
      ...
      <fieldset className="space-y-3 border-0 p-0 m-0">
        <legend className="text-xs text-slate-400 font-semibold">{t('wizard.step1')}</legend>
        ...
        <>
          ...
          <fieldset className="space-y-2 border-0 p-0 m-0">
            ...
            <div className="space-y-2">
              ...
              {preview.roster.length === 0 && <p className="text-xs text-slate-400">{t('wizard.noCreatures')}</p>}
            </div>
          </fieldset>

          {/* ── Step 3: commit ── */}
          <fieldset className="space-y-3 border-0 p-0 m-0">
            <legend className="text-xs text-slate-400 font-semibold">{t('wizard.step3')}</legend>
            <Field
              idPrefix="gen-enc"
              name="name"
    ...
    <div className="rounded-lg border border-white/10 p-2">
      ...
      {expanded && (
        <div className="mt-2 text-xs text-slate-300 grid gap-1">
          ...
          {!insp.hasStatblock && <p className="text-amber-300">{t('wizard.noStatblock')}</p>}
        </div>
      )}
    </div>
  );
