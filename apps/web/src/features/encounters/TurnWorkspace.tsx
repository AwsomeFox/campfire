  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1">
        <button type="button" className="btn btn-ghost !min-h-[24px] !py-0.5 text-[11px]" disabled={disabled} onClick={onUse}>
          {isMovement ? '+5 ft' : 'Use'}
        </button>
        <button type="button" className="btn btn-ghost !min-h-[24px] !py-0.5 text-[11px]" disabled={disabled || slot.used <= 0} onClick={onRelease}>
    ...
    <Card className="space-y-3">
      ...
      <div className="flex gap-4 flex-wrap text-sm">
        ...
        <span className="text-muted">
          ...
          {turn.concentration && (
            <button type="button" className="btn btn-ghost !min-h-[24px] !py-0.5 text-[11px] ml-1" disabled={controlsDisabled} onClick={() => turnState.mutate({ concentration: null })}>
              clear
            </button>
          )}
        </span>
          ...
          <ul className="list-none p-0 m-0 space-y-1">
            {turn.activeEffects.map((e) => (
              <li key={e.id} className="text-sm text-muted flex items-center gap-2">
                ...
                <button type="button" className="btn btn-ghost !min-h-[24px] !py-0.5 text-[11px]" disabled={controlsDisabled} onClick={() => turnState.mutate({ removeEffectId: e.id })}>
                  remove
                </button>
              </li>
            ))}
