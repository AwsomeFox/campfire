...
type LinkDraft = { sessionId: number | null; questId: number | null; encounterId: number | null };
const LINK_CONFLICT_FIELDS: Array<ConflictField<LinkDraft>> = [
  { key: 'sessionId', label: 'Linked session' },
  { key: 'questId', label: 'Linked quest' },
  { key: 'encounterId', label: 'Linked encounter' },
];
...
function BeatRow({
  ...
  const [linkDraft, setLinkDraft] = useState<LinkDraft>({ sessionId: beat.sessionId, questId: beat.questId, encounterId: beat.encounterId });
  const [linkBase, setLinkBase] = useState<LinkDraft>({ sessionId: beat.sessionId, questId: beat.questId, encounterId: beat.encounterId });
  const [linkExpected, setLinkExpected] = useState(beat.updatedAt);
  const [linkConflict, setLinkConflict] = useState<{ base: LinkDraft; theirs: LinkDraft } | null>(null);
  ...
  const saveLinks = async () => {
    ...
    setLinkError(null);
    try {
      await api.patch(`${API}/beats/${beat.id}`, { ...linkDraft, expectedUpdatedAt: linkExpected });
      ...
      if (isStaleWrite(err)) {
        try {
          ...
          const theirs = { sessionId: latest.sessionId, questId: latest.questId, encounterId: latest.encounterId };
          setLinkConflict({ base: linkBase, theirs });
          setLinkBase(theirs);
          setLinkExpected(latest.updatedAt);
        } catch {
          setLinkError("The beat changed, but the latest links couldn't be loaded. Your choices are still here.");
        }
      } else setLinkError("Couldn't save the beat links.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div style={{ borderLeft: '2px solid var(--color-border)', paddingLeft: 10, display: 'flex', flexDirection: 'column', gap: 5 }} {...entityTargetProps('beat', beat.id)}>
      ...
      {isDm &&
        (editingLinks ? (
          <div style={{ display: 'flex', gap: 6, flexDirection: 'column', marginLeft: 22 }}>
            {linkError && <ErrorNote message={linkError} />}
            {linkConflict && (
              <StaleWriteConflict
                base={linkConflict.base}
                mine={linkDraft}
                theirs={linkConflict.theirs}
                fields={LINK_CONFLICT_FIELDS}
                onResolve={(key, value) => setLinkDraft((draft) => ({ ...draft, [key]: value }))}
                onReloadAll={() => { setLinkDraft(linkConflict.theirs); setLinkBase(linkConflict.theirs); setLinkConflict(null); }}
              />
            )}
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
            <select
              className="input"
              value={linkDraft.sessionId != null ? String(linkDraft.sessionId) : ''}
              disabled={busy}
              onChange={(e) => setLinkDraft((draft) => ({ ...draft, sessionId: e.target.value ? Number(e.target.value) : null }))}
            ...
            <select
              className="input"
              value={linkDraft.questId != null ? String(linkDraft.questId) : ''}
              disabled={busy}
              onChange={(e) => setLinkDraft((draft) => ({ ...draft, questId: e.target.value ? Number(e.target.value) : null }))}
              style={{ fontSize: 12, width: 'auto' }}
            ...
            <select
              className="input"
              value={linkDraft.encounterId != null ? String(linkDraft.encounterId) : ''}
              disabled={busy}
              onChange={(e) => setLinkDraft((draft) => ({ ...draft, encounterId: e.target.value ? Number(e.target.value) : null }))}
              style={{ fontSize: 12, width: 'auto' }}
            ...
            <button className="btn btn-primary" style={{ fontSize: 11 }} disabled={busy} onClick={() => void saveLinks()}>
              {linkConflict ? 'Save resolution' : 'Save links'}
            </button>
            <button className="btn btn-ghost" style={{ fontSize: 11 }} disabled={busy} onClick={() => { setEditingLinks(false); setLinkConflict(null); }}>
              Cancel
            </button>
            </div>
          </div>
          ...
          <button
            ...
            disabled={busy}
            onClick={() => {
              const current = { sessionId: beat.sessionId, questId: beat.questId, encounterId: beat.encounterId };
              setLinkDraft(current);
              setLinkBase(current);
              setLinkExpected(beat.updatedAt);
              setLinkConflict(null);
              setLinkError(null);
