...
!!! warning "Approving the waiting actions needs the tool-confirmation UI"
    Deferred calls land in the AI's pending tool-confirmation queue. **That queue has no web
    interface yet** (issue #1558) — today it can only be resolved through the API. Until that
    ships, actions the AI defers will sit unapproved, and the fiction will run ahead of the
    board. Treat collaborative handoff as unfinished until #1558 lands.
