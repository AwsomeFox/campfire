...
| Audit admin actions | ✅ | A **server-wide audit** (**Admin → Audit**, `/admin/audit`) of admin actions not tied to a campaign — account creation, settings changes, pack installs — alongside the per-campaign log. The same page discloses audit retention (default 365 days, auto-prune off unless enabled), supports dry-run counts, legal hold, archive-before-prune, backup freshness checks, and last-job status. |
