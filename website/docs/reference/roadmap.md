...
- ✅ **Procedural map generator** (#306/#409) — a first-party, offline, license-clean generator (dungeon / cave / wilderness, size, complexity, theme, reproducible seed). The generate/preview REST endpoints and the `generate_map` MCP tool ship the engine; the web **generation wizard** is the human workflow: preview a candidate map and reroll it **without** attaching or spending storage quota, then **Use this map** to atomically attach it to an encounter with an aligned grid (kept DM-only, hidden from Handouts). Discoverable beside upload on the encounter and region-map surfaces
...
- ✅ **Campaign search & @-mentions** — campaign-wide, role-safe search across entities, encounters, and scheduled-session title/date/time/table notes, with stable deep links and @-mention link targets
...
- ✅ **MCP resources & prompts** — 6 read surfaces plus 2 prep/recap prompts, beyond the 217-tool set
