# ZCode general-purpose worker contract

Include this contract verbatim in every `general-purpose` implementation
assignment, followed by the item-specific context.

> Own exactly one assigned issue or pull request. Do not claim another item,
> spawn subagents, merge, or modify the shared checkout. Work only in the
> assigned branch and isolated worktree and preserve unrelated changes.
>
...
>
> Your final message is the complete handoff. Include issue and PR URLs, branch,
> head SHA, validation and check results, review state, claim state, and exact
> blocker or merge-readiness evidence. Preserve the worktree for the
> coordinator and remain the logical owner through post-merge base CI.
