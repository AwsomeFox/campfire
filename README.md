...
- **AI-operable from day 1** — the same service layer is exposed as a REST API (OpenAPI) and an MCP server (224 tools), so an AI assistant can maintain — or run — the campaign; AI writes can be routed through a DM-approved proposal queue
...
> server with **224 tools**, so any MCP-capable client (e.g. Claude, via `claude mcp
...
Shipped: entities + notes + OpenAPI, OIDC/roles, MCP server (224 tools), media & maps
