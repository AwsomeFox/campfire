...
<<<<<<< HEAD
- **AI-operable from day 1** — the same service layer is exposed as a REST API (OpenAPI) and an MCP server (261 tools), so an AI assistant can maintain — or run — the campaign; AI writes can be routed through a DM-approved proposal queue
=======
- **AI-operable from day 1** — the same service layer is exposed as a REST API (OpenAPI) and an MCP server (262 tools), so an AI assistant can maintain — or run — the campaign; AI writes can be routed through a DM-approved proposal queue
>>>>>>> 7941b540c (feat(mcp): expose player AoE template tools)
...
> server with **262 tools**, so any MCP-capable client (e.g. Claude, via `claude mcp
...
Shipped: entities + notes + OpenAPI, OIDC/roles, MCP server (262 tools), media & maps
